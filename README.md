...
### Upgrading to 0.8.0 (and 0.7.0 — treat them as one migration)

**If you are coming from 0.6.x, do both steps below at once.** 0.7.0 and 0.8.0 renamed on-disk paths in two consecutive releases, and nothing carries the old names forward, so skipping straight from 0.6.x means one migration rather than two. The end state is the only thing you need:
...
0.7.0 moved everything out of `.claude-sesh-mover/`, and split the per-project state by **lifecycle**: generated exports (which must never be committed) went to `.sesh-mover/`, while config and identity (which must be committed, or they do nothing) left that directory entirely to become root dotfiles. That is why a plain `.sesh-mover/` line in `.gitignore` now works with no negation.

0.8.0 then dropped `hub` from the two list files: `.sesh-mover-hubinclude` → `.sesh-mover-include`, `.sesh-mover-hubignore` → `.sesh-mover-ignore`. The hub used to be the only thing that read them; it is not any more, so a name asserting "hub" was wrong for anyone syncing without one. `.sesh-mover-project.json` is genuinely hub-specific and is **unchanged**.

**The user directory moves itself. Project files do not.** On its first run after 0.7.0, sesh-mover renames `~/.claude-sesh-mover/` to `~/.sesh-mover/` in a single move, so this machine keeps its hub identity (`machine-id.json`) and every project's sync state — nothing to re-link, no threads orphaned. If both directories already exist it touches neither and warns, leaving you to merge them; if the rename fails it keeps using the old directory and warns rather than starting empty.

**Project-level files are never migrated, and old names are never read as a fallback.** Rename them yourself — `git mv .sesh-mover-hubinclude .sesh-mover-include` and the same for the ignore list — or recreate them by hand if you are coming from a `.claude-sesh-mover/` directory. A file left under an old name is simply not read, so the project syncs as if it had no list at all; that is recoverable, whereas a half-read list would not be. Copying `project.json` across verbatim preserves that project's link to its hub project; leaving it behind means the next `/sesh-mover:push` re-links by git remote, or offers to mint a fresh hub project. **Every old name stays on the never-syncable floor permanently** (below) — bundles carrying them are already sitting on hubs and inside export archives, and dropping a name would un-protect exactly those.
...
- The floor can never be applied, whatever a bundle says: `.git`, and every path sesh-mover owns (`.sesh-mover/`, the `.sesh-mover-*` root dotfiles, and the retired names `.claude-sesh-mover/`, `.sesh-mover-hubinclude` and `.sesh-mover-hubignore`) — at any depth, in any casing (`workspaceRefused`).
