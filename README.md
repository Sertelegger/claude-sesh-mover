...
**Keys.** Each machine mints an X25519 identity at `~/.sesh-mover/identity.age` (mode 0600) on first use. The private half is **never transported**; the public half is published in that machine's hub record on every push and pull, which is what makes the recipient list derivable from the hub itself. Joining a machine is `hub init` there — no key exchange, no flag day. A bundle is encrypted to every machine registered on the hub at the moment it is written, which has one consequence worth knowing up front: **a machine that joins later gets new bundles immediately and does not retroactively gain access to old ones.**
