...
What a pull still cannot deliver, it names rather than swallowing: a bundle whose recorded link points at an entry no bundle on the hub carries, a branch it did not follow when a thread forks, a bundle pushed **before** this release recorded links at all (unlinkable by construction, not missing — it re-links the next time that machine pushes), and a machine advertising newer work it has never uploaded. Those are reported as `unfetchableBundles` plus a warning saying which of them applies. A pull that finds one of these and correctly applies nothing is a **success** that says why, not an error you can retry.
