import { mkdirSync, openSync, closeSync, writeSync, rmSync, readFileSync, statSync, renameSync, } from "node:fs";
...
export class LockBusyError extends Error {
     ...
     */
    stealEligibleInMs;
    constructor(holderPid, ageMs, holderLiveness = "unknown", stealEligibleInMs = null) {
 ...
 * shape of it.
 */
export function probeHolderLiveness(pid, host) {
 ...
 */
function writeStealRecord(lock, record) {
...
function readStealRecord(lock) {
    try {
        const parsed = JSON.parse(readFileSync(stealRecordPath(lock), "utf-8"));
 ...
 */
export function readLockStealRecord(projectPath) {
...
export function acquireProjectLock(projectPath) {
    ...
    if (fd === null) {
        ...
        if (ageMs !== null && ageMs > threshold) {
            ...
            if (fd === null) {
                ...
                disclose(`sesh-mover: took the project lock for ${projectPath} from ` +
        ...
        if (fd === null) {
            throw new LockBusyError(holderPid, ageMs, liveness, ageMs === null ? null : Math.max(0, threshold - ageMs));
        }
    }
    ...
    const noticeSteal = () => {
        try {
            ...
            if (rec === null || rec.holderToken !== token || rec.noticedByHolderAt)
                return;
            writeStealRecord(p, { ...rec, noticedByHolderAt: new Date().toISOString() });
            disclose(`sesh-mover: this process (pid ${process.pid}) lost the project lock for ${projectPath}` +
 ...
 */
export function describeLockSteal(steal, verb) {
    ...
    const who = steal.kind === "live-holder-past-ceiling"
    ...
    return (`Stole the project lock from ${who} after it held it past the safety ceiling \u2014 the ${verb} proceeded, ` +
