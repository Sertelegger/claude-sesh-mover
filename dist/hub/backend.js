    ...
    return {
        ...
        async list(relPrefix) {
            ...
            const walk = async (dir, rel) => {
                ...
                const entries = await withHubIoTimeout("readdir", () => readdir(dir, { withFileTypes: true }));
        ...
        async readStream(relPath) {
            ...
            const handle = await withHubIoTimeout("open", () => open(abs(relPath), "r"), (fh) => void fh.close().catch(() => { }));
        ...
        async writeStreamAtomic(relPath) {
            ...
            await withHubIoTimeout("mkdir", () => mkdir(dirname(target), { recursive: true }));
            const tmp = `${target}.tmp-${randomUUID()}`;
            const handle = await withHubIoTimeout("open", () => open(tmp, "w"), (fh) => void fh.close().catch(() => { }));
