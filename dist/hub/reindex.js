...
import { digestFile, signStatement } from "./signature.js";
...
function recoverPushedAt(sanitizedIso) {
...
export async function hubReindex(opts) {
    ...
    try {
        ...
        // prose.
        const unrecognizedBundleFiles = [];
        const droppedBundles = [];
        let droppedFromPriorIndex;
        ...
        const syntheticPrior = { threads: Object.fromEntries(threadMeta) };
        ...
        if (priorIndex !== null) {
            const rebuilt = new Set(Object.values(built.threads).flatMap((t) => t.bundles.map((b) => b.bundleId)));
...
async function rebuildSignature(args) {
    try {
        ...
        if (!key.ok) {
            args.warnings.push(`this machine's signing key could not be read (${key.detail}), so rebuilt records carry no signature. Every other machine will read them as unsigned until this machine pushes again with a working key.`);
            return null;
        }
        let workspaceFile;
        let workspaceDigest;
        ...
        return signStatement(key.privateKey, {
