...
async function fetchAncestorWorkspace(backend, ref, tempRoot) {
    ...
    try {
        ...
        if (!got.ok)
            return degraded(`could not be read back (${got.failure.message})`);
...
export async function runApplyWorkspaceStage(input) {
    ...
    if (bundleDeclaresWorkspace && !isReadableDir(incomingDir)) {
        ...
        if (ancestorDir !== null) {
            ...
            try {
                const ws = await unpackWorkspace(incomingDir, effectiveProjectPath, { force: forceWorkspace || !hasRealContent });
