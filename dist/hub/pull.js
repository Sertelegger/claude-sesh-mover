import { mkdtempSync, rmSync, mkdirSync, createReadStream, existsSync, readdirSync, appendFileSync, copyFileSync, statSync, } from "node:fs";
...
import { readSyncState, peekSyncState, writeSyncState, getThreadId, setThreadId, recordSentToPeer, } from "../sync-state.js";
/**
...
export async function hubPull(opts) {
    ...
    try {
         ...
         */
        const alternateSource = (t, st) => {
            const candidates = t.copies.filter((c) => c.machineId !== machine.id &&
                selectNeededBundles(c.bundles, st.peers[c.machineId]?.received, (id) => existsSync(join(targetProjectDir, `${id}.jsonl`))).length > 0);
        ...
        if (opts.threadId) {
            ...
            target =
                resolved.find((t) => !isCurrent(t)) ??
                    resolved.find((t) => t.latest.machineId === machine.id &&
        ...
        for (const [i, record] of needed.entries()) {
            ...
            if (record.type === "continuation" && !opts.noAppend && bundleSession && existsSync(deltaPath)) {
                ...
                if (actualHash !== bundleSession.integrityHash) {
                    ...
                    const baseSessionId = selectThreadBase(threadBaseCandidates(readSyncState(effectiveProjectPath), target.threadId, st.threadLandedSessionId, targetProjectDir), anchorUuid, st.threadLandedSessionId);
                    if (baseSessionId) {
                        ...
                        if (outcome.reason === "chain-mismatch") {
                            ...
                            const preventive = outcome.reason === "recently-active"
                                ...
                                : "";
                            warnings.push(`Continuation for thread ${target.threadId} could not be appended to the local session (${outcome.detail}) — imported as a separate session instead.${preventive}`);
                        }
                    }
                }
            }
        ...
        if (st.divergenceAborted) {
            ...
            if (st.deferredBundles > 0) {
                parts.push(`the ${st.deferredBundles} later bundle${st.deferredBundles === 1 ? "" : "s"} in this thread's chain ${st.deferredBundles === 1 ? "was" : "were"} not fetched`);
            }
            if (carrySuppressed) {
                parts.push("the uncommitted work that bundle carried was left in it");
            }
            if (parts.length > 0) {
                warnings.push(`Because that decision is still open, ${parts.join(" and ")} — nothing from ${parts.length === 1 ? "it" : "them"} was applied, saved or recorded either.` +
                    (st.abortIndex > 0
