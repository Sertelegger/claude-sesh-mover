 ...
 */
function checkHubReachable(command, hubPath, hub, rootIsDir) {
    if (!rootIsDir) {
        return {
            success: false,
            command,
            ...
            suggestion: "hub.path names a directory this machine cannot see. If the hub is a network share, check that it is mounted; if it is a synced folder, it may not have reached this machine yet. Nothing was read from or written to the hub.",
        };
    }
    if (hub === null || typeof hub.hubId !== "string" || hub.hubId.length === 0) {
        return {
            ...
            reason: "hub-unreachable",
            hubState: "not-a-hub",
            suggestion: "The configured hub directory is readable but carries no usable hub.json, so this machine cannot tell which hub it is. If the hub is a synced folder its first sync may still be in flight; otherwise hub.path is set to a directory that is not a sesh-mover hub. Nothing was written.",
        };
    }
    return null;
}
