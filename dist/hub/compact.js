...
import { compactionDirPath, compactionPath, } from "./layout.js";
 ...
 */
async function collectEntryUuids(jsonlPaths) {
...
export function localSessionsForThread(state, threadId) {
 ...
 */
export function findConsolidated(records) {
    for (let i = records.length - 1; i >= 0; i--) {
        if (records[i].type === "full")
            return records[i];
    }
    return null;
}
/** Strip records from one thread of a prior index, leaving everything else. */
function withoutRecords(index, threadId, removeBundleIds) {
...
async function readCompactionFile(backend, projectId, machineId) {
    ...
    try {
        ...
        if (!Array.isArray(parsed?.compactions))
            return empty;
 ...
 * not.
 */
export function pruneCompactions(compactions, indexes) {
    const referenced = new Set();
    for (const index of indexes) {
        for (const entry of Object.values(index.threads)) {
            for (const r of entry.bundles)
                referenced.add(r.bundleId);
        }
    }
 ...
 * not tell the victim.
 */
export async function hubCompact(opts) {
    ...
    if (probe.state !== "ok")
        return hubUnreachableRefusal("hub-compact", probe.state);
    ...
    if (!ownIndex || !entry) {
        return {
            ...
            suggestion: "Compaction consolidates from the machine that holds the thread. Run `sesh-mover whereis` to see which machine that is, and compact there — this one has nothing of its own to retire.",
        };
    }
    ...
    if (lastFullAt === -1 || lastFullAt < entry.bundles.length - 1) {
        ...
        if (sessionIds.length === 0) {
            return {
                ...
                suggestion: "A consolidated bundle is a fresh FULL export of the transcript, so it can only be produced by a machine that holds one. Pull the thread here first, or compact on the machine that has it.",
            };
        }
        ...
        warnings.push("Nothing has been deleted. The consolidated bundle is on the hub and is already what a reader fetches; the chain behind it stays until every machine has demonstrably got the content and the grace window has passed. Re-run this command later to finish the job.");
        return {
            success: true,
            command: "hub-compact",
            phase: "consolidated",
    ...
    const suggestionForPending = "Nothing is broken while this is the answer: the consolidated bundle is on the hub and is already what a reader fetches. Re-run this command when the machines below have pulled.";
    ...
    const threadSessions = new Set(localSessionsForThread(peekSyncState(opts.projectPath), opts.threadId));
    const uuids = await collectEntryUuids(sessions.filter((s) => threadSessions.has(s.sessionId)).map((s) => s.jsonlPath));
    ...
    if (ack.outstanding.length > 0 || nowMs < eligibleAtMs) {
        ...
        if (nowMs < eligibleAtMs) {
            warnings.push(`The consolidated bundle is younger than the ${Math.round(COMPACTION_GRACE_MS / 3_600_000)}-hour grace window. That window is not about propagation — acknowledgement already proves receipt — it is the only defence against a pull that is in flight RIGHT NOW, and against a machine whose interrupted pull left it holding content it never published.`);
        }
        return {
            success: false,
            command: "hub-compact",
            ...
            threadId: opts.threadId,
            consolidatedBundleId: consolidated.bundleId,
            outstanding: await Promise.all(ack.outstanding.map(async (o) => ({
...
async function retirePhase(args) {
    ...
    try {
        ...
        if (e instanceof LockBusyError) {
            return {
                ...
                error: e.message,
                suggestion: "Another sesh-mover hub operation is running for this project — wait for it and re-run. There is deliberately no override here: what it would be racing is a push writing new bundles into the very thread being compacted.",
            };
        }
        throw e;
    }
    try {
        ...
        await backend.writeAtomic(compactionPath(projectId, me.id), JSON.stringify(marker, null, 2) + "\n");
        ...
        for (const r of doomed) {
            ...
            if (await backend.exists(artifact))
                await removeFile(artifact);
        }
        if (failed.length > 0) {
            warnings.push(`${failed.length} file(s) could not be removed and are listed in \`failed\`. They are no longer referenced by this machine's index, so nothing reads them; re-run to finish the job.`);
        }
        warnings.push("Only THIS machine's bundles were retired. If this thread's history was pushed from more than one machine, each machine reclaims its own share by running this command there — one machine deleting another's files is what per-machine ownership forbids, and it is what makes concurrent push and pull safe with no distributed lock.");
        return {
            success: true,
            command: "hub-compact",
            phase: "retired",
 ...
 */
export async function readAllCompactions(backend, projectId) {
    ...
    for (const file of files) {
        ...
        if (name.includes("/") || !name.endsWith(".json"))
            continue;
        try {
            const parsed = JSON.parse((await backend.read(file)).toString());
            if (!Array.isArray(parsed?.compactions))
                continue;
            for (const c of parsed.compactions) {
                if (Array.isArray(c?.retiredBundleIds))
                    out.push(c);
            }
        }
        catch {
 ...
 * the remedy.
 */
export async function explainRetiredBundles(backend, projectId, bundleIds) {
    ...
    try {
        for (const c of await readAllCompactions(backend, projectId)) {
            for (const id of c.retiredBundleIds) {
                if (wanted.has(id))
                    explained.set(id, c.consolidatedBundleId);
            }
        }
    }
    catch {
