import { chmodSync, closeSync, copyFileSync, mkdirSync, mkdtempSync, openSync, readFileSync, readSync, renameSync, rmSync, statSync, unlinkSync, writeFileSync, } from "node:fs";
...
export class MergeAncestorRequiredError extends Error {
    constructor() {
        super("a 3-way workspace merge needs an ancestor tree; with no known ancestor use " +
 ...
 */
export async function isGitMergeFileAvailable() {
    let dir;
    try {
    ...
    try {
        ...
        const status = await new Promise((resolve) => {
            execFile("git", [...GIT_MERGE_ARGS, current, base, other], { cwd: dir, env: gitChildEnv(), timeout: PROBE_TIMEOUT_MS, windowsHide: true }, (err) => {
                if (err === null)
                    return resolve(0);
                const code = err.code;
...
export async function mergeWorkspaceTrees(opts) {
    if (opts.ancestorDir === null)
        throw new MergeAncestorRequiredError();
    const report = {
    ...
    const sidecar = (rel, incomingPath, reason, detail) => {
        ...
        for (const rel of [...all].sort()) {
            try {
                ...
                if (incomingPath === null) {
                    ...
                    if (localPath !== null && ancestorPath !== null)
                ...
                if (sameContent(localPath, incomingPath))
                    continue; // incl. the same edit on both sides
