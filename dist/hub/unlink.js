 ...
 */
export function hubUnlink(opts) {
    const path = projectJsonFilePath(opts.projectPath);
    const warnings = [];
    let lock = null;
    if (opts.force) {
        warnings.push("The project lock was skipped (--force), so a sesh-mover hub operation that is genuinely still running for this project could write the link file again when it finishes — check with `sesh-mover hub status` afterwards if a push or pull may have been in flight.");
    }
    else {
        try {
            ...
            if (e instanceof LockBusyError) {
                ...
                return {
                    ...
                    error: e.message,
                    suggestion: "Another sesh-mover hub operation is running for this project — wait for it and try again, or pass --force to unlink without waiting for it (a push that is still running could then re-create the link when it finishes).",
                };
            }
            throw e;
        }
    ...
    try {
        ...
        if (stat.isDirectory()) {
            ...
            return {
                ...
                suggestion: "Inspect that path by hand. Nothing sesh-mover writes creates a directory there, so this was not made by the plugin.",
            };
        }
        ...
        try {
            ...
            const id = parsed !== null && typeof parsed === "object"
                ? parsed.projectId
                : undefined;
        ...
        if (!projectId) {
            warnings.push("The link file could not be read as a hub identity (corrupt, empty, or not JSON), so the hub project id it named could not be reported. It was removed anyway: a file no reader understands is still a file sitting in this project.");
        }
        try {
        ...
        warnings.push("This project's local sync bookkeeping was kept, so nothing has to be re-uploaded if you link this directory to the same hub project again.");
        if (projectId) {
        ...
        warnings.push("Unlinking applies to this directory only. Any other directory holding a .sesh-mover-project.json — a scratch directory a pull unpacked a workspace into, for instance — is still linked and still armed, and has to be unlinked on its own.");
        warnings.push("Sessions this project already pushed stay on the hub, and no other machine is affected: unlinking removes one local file and writes nothing to the hub.");
        return {
            success: true,
            command: "hub-unlink",
            wasLinked: true,
