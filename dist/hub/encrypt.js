 ...
 */
export function isEncryptionCapableVersion(recorded) {
    if (typeof recorded !== "string" || !/^\d+(?:\.\d+)*$/.test(recorded.trim()))
        return false;
    return compareVersions(recorded.trim(), MIN_ENCRYPTION_PLUGIN_VERSION) >= 0;
}
function describeStale(m) {
 ...
 * assumption.
 */
async function findStaleMachines(backend, exemptMachineId) {
    const stale = [];
    for (const machineId of await listMachineIds(backend)) {
        if (machineId === exemptMachineId)
            continue;
        ...
        if (isEncryptionCapableVersion(record.pluginVersion))
            continue;
        stale.push({
            machineId,
...
export async function hubEncrypt(opts) {
    ...
    if (opts.enable) {
        const configDir = opts.configScope === "project" ? projectSeshMoverDir(opts.cwd) : userSeshMoverDir();
        try {
            writeConfigOverrides(configDir, setConfigOverride(readConfigOverrides(configDir), "hub.encrypt", true));
            preference = true;
        }
        catch (e) {
    ...
    if (policy.malformedSetting) {
        ...
        warnings.push("This hub's hub.json has an `encrypt` value that is neither true nor false. Every machine reads it as ENCRYPTED, because a hand-edited `\"true\"` read the other way is a silent confidentiality loss. Fix the value on the hub to settle it.");
    }
    if (changed) {
        warnings.push("Enabling encryption does not make this hub private. It makes it private GOING FORWARD: every bundle already on the hub stays readable by anyone with read access to the directory, forever, and nothing here rewrites them — one machine rewriting another machine's files is what per-machine ownership forbids. If the existing bundles matter, start a fresh hub.");
