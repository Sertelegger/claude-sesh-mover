import { tombstoneDirPath, tombstonePath, } from "./layout.js";
...
export function graceState(retiredAt, nowMs) {
    const at = Date.parse(retiredAt);
    if (Number.isNaN(at))
        return { eligibleAt: null, remainingMs: null, eligible: false };
...
function parseTombstone(raw, machineIdFromName) {
    let parsed;
    try {
    ...
    if (parsed === null || typeof parsed !== "object")
        return null;
    if (typeof parsed.retiredAt !== "string" || parsed.retiredAt.length === 0)
        return null;
    return {
        schemaVersion: 1,
 ...
 */
export async function readTombstones(backend, projectId) {
...
export async function readTombstone(backend, projectId, machineId) {
...
export async function writeTombstone(backend, tombstone) {
    await backend.writeAtomic(tombstonePath(tombstone.projectId, tombstone.machineId), JSON.stringify(tombstone, null, 2) + "\n");
}
/**
 ...
 * fires on it.
 */
export function primaryTombstone(tombstones) {
    let best = null;
 ...
 */
export async function findRetirement(backend, projectId) {
