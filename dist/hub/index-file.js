...
export function buildIndexFile(inputs) {
    ...
    const priorBundles = (threadId) => priorThreads && Object.hasOwn(priorThreads, threadId)
...
export async function readMachineIndex(backend, projectId, machineId, warnings) {
    ...
    if (parsed.machineId !== machineId) {
        warnings?.push(`${where}: the file declares machineId ${JSON.stringify(parsed.machineId)} — skipped; ` +
