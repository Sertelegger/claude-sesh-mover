 ...
 */
import { createCipheriv, createDecipheriv, createHmac, createPrivateKey, createPublicKey, diffieHellman, hkdfSync, randomBytes, scryptSync, timingSafeEqual, } from "node:crypto";
...
export function parseRecipient(s) {
    ...
    try {
        ...
        throw new AgeError("bad-key", `not an age recipient: ${e.message}`);
    }
    if (hrp !== RECIPIENT_HRP)
        throw new AgeError("bad-key", `not an age recipient: hrp ${hrp}`);
...
export function parseIdentity(s) {
    ...
    try {
        ...
        throw new AgeError("bad-key", `not an age identity: ${e.message}`);
    }
...
export function recipientFromIdentity(identityRaw) {
...
function unb64(s) {
    if (s.includes("="))
        throw new AgeError("malformed-header", "base64: padding is not allowed");
 ...
 */
function wrapFileKey(fileKey, recipientRaw) {
    ...
    if (shared.every((b) => b === 0))
        throw new AgeError("bad-key", "all-zero X25519 shared secret");
 ...
 */
function unwrapFileKey(share, body, identityRaw) {
    if (share.length !== X25519_KEY_SIZE || body.length !== FILE_KEY_SIZE + TAG_SIZE)
        return null;
 ...
 */
function headerMac(fileKey, headerUpToMark) {
...
function parseHeader(header) {
    ...
    if (markIdx === -1)
        throw new AgeError("malformed-header", "no header MAC line");
    const macLineEnd = text.indexOf("\n", markIdx + 1);
    if (macLineEnd === -1)
        throw new AgeError("malformed-header", "truncated header MAC line");
    ...
    for (let i = 1; i < lines.length; i++) {
        const line = lines[i];
        if (!line.startsWith("-> "))
            throw new AgeError("malformed-header", `expected a stanza at line ${i}`);
        ...
        while (i + 1 < lines.length) {
            const b = lines[++i];
            bodyLines.push(b);
            if (b.length < STANZA_WRAP_COLUMNS)
                break;
        }
 ...
 */
function verifyHeaderMac(fileKey, parsed) {
 ...
 */
function scryptWrapKey(passphrase, salt, logN) {
...
function buildScryptHeader(fileKey, passphrase, logN) {
 ...
 */
function openHeaderWithPassphrase(header, passphrase) {
    const parsed = parseHeader(header);
 ...
 */
function streamNonce(counter, final) {
    const nonce = Buffer.alloc(12);
    if (counter >= 1n << 88n)
        throw new AgeError("truncated", "STREAM counter overflow");
    let v = counter;
    for (let i = 10; i >= 0; i--) {
...
function sealChunk(payloadKey, counter, final, plain) {
...
function openChunk(payloadKey, counter, final, enc) {
    if (enc.length < TAG_SIZE)
        return null;
...
export class AgeDecryptStream extends Transform {
    ...
    _transform(chunk, _enc, cb) {
        try {
            ...
            while (this.pending.length > ENC_CHUNK_SIZE) {
                ...
                if (!plain) {
                    return cb(new AgeError("payload-authentication-failed", `chunk ${this.counter} failed authentication`));
                }
                if (plain.length > 0)
                    this.push(plain);
                this.counter++;
    ...
    _flush(cb) {
        try {
            ...
            if (!plain) {
                return cb(new AgeError("payload-authentication-failed", `final chunk ${this.counter} failed authentication`));
            }
            if (plain.length > 0)
                this.push(plain);
            cb();
