...
hub
    ...
    try {
        ...
        if (!opts.thread) {
            ...
            outputError("hub-compact", new Error("Which thread? Pass --thread <id>. Run `sesh-mover whereis` to list this project's threads — compaction deletes bundles, so it never guesses."));
            return;
        }
...
hub
    ...
    try {
        ...
        let passphrase = { kind: "not-requested" };
...
async function readPassphraseFromStdin() {
    ...
    for await (const c of process.stdin) {
        ...
        total += b.length;
        if (total > LIMIT) {
            throw new Error(`Refusing to read more than ${LIMIT} bytes as a passphrase. ` +
