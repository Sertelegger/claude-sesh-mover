 ...
 */
export function foreignKeyedRecord(from) {
 ...
 * runs.
 */
export function peekSyncState(projectPath) {
    const p = syncStatePath(projectPath);
    if (!existsSync(p))
        return defaultState(projectPath);
    try {
 ...
 */
export function setPeerMemoryDigest(state, peer, digest) {
    if (!state.peers[peer.id]) {
        state.peers[peer.id] = {
            name: peer.name ?? peer.id,
            lastSentAt: null,
 ...
 */
export function forgetSentToPeer(state, peer, opts) {
    ...
    entry.sent = sent;
    // Asked for explicitly, or implied by the whole-ledger form below. Read
    // before the scoped branch returns so both paths report it the same way.
    const dropMemory = opts?.memoryDigest === true;
    ...
    if (scope) {
        ...
        for (const id of scope) {
            ...
            if (!Object.prototype.hasOwnProperty.call(sent, id))
                continue;
            delete sent[id];
            forgotten.push(id);
        }
        const memoryDigest = dropMemory && entry.memoryDigest !== undefined;
        if (dropMemory)
 ...
 */
export function setLastAutoPush(state, entry) {
    if (!state.hub)
        return;
