import { closeSync, createWriteStream, existsSync, mkdirSync, mkdtempSync, openSync, readFileSync, readSync, rmSync, statSync, } from "node:fs";
 ...
 */
export function zstdDecompressionLimit(compressedBytes) {
    return Math.max(MIN_ZSTD_DECOMPRESSION_ALLOWANCE, compressedBytes * MAX_ZSTD_DECOMPRESSION_RATIO);
}
/**
...
export class ZstdNoContentChecksumError extends Error {
    constructor(archivePath) {
        super(`zstd produced ${basename(archivePath)} without a frame content checksum, so corruption in it could not be detected on extraction`);
    }
}
/**
 ...
 */
export function zstdFrameHasContentChecksum(archivePath) {
    let fd;
    try {
 ...
 */
export async function extractArchive(archivePath, targetDir, warnings) {
 ...
 */
async function scanBundleEntries(tarFile) {
    const offenders = [];
    let manifestSize = null;
    await tar.list({
        file: tarFile,
        onReadEntry: (entry) => {
 ...
 */
function isBundleRootManifest(p) {
 ...
 * throw here.
 */
async function decompressZstd(archivePath, tarPath) {
    ...
    if (overran) {
        ...
        throw new Error(`${basename(archivePath)} expands to more than ${limit} bytes — over ${MAX_ZSTD_DECOMPRESSION_RATIO}x its ${compressedBytes} compressed bytes, which no session bundle is — so it was not decompressed`);
    }
    ...
    if (code !== 0) {
        ...
        throw new Error(`zstd could not decompress ${basename(archivePath)} (exit ${code ?? "signal"})${detail ? `: ${detail}` : ""}`);
    }
}
/**
...
async function extractZstdArchive(archivePath, targetDir, warnings) {
    const workDir = mkdtempSync(join(tmpdir(), "sesh-mover-zstd-"));
    const tarPath = join(workDir, "bundle.tar");
    try {
        ...
        if (zstdFrameHasContentChecksum(archivePath) === false) {
            warnings?.push(`${basename(archivePath)} was compressed without a zstd frame checksum, so the archive itself cannot show whether it was corrupted in transit. The bundle's own manifest hashes were still verified.`);
        }
