...
import { readFirstJsonlLine, readFirstConversationEntry, readLastConversationEntry, countJsonlLines, } from "./jsonl.js";
export function discoverSessions(configDir, projectPath) {
    ...
    for (const { encoded: encodedDir, dir: projectDir } of dirs) {
        ...
        for (const file of files) {
            ...
            const session = parseSessionJsonl(jsonlPath, sessionId, projectPath, encodedDir, configDir);
            if (session) {
...
function parseSessionJsonl(jsonlPath, sessionId, projectPath, encodedProjectDir, configDir) {
    try {
        ...
        const mtimeIso = () => statSync(jsonlPath).mtime.toISOString();
        const createdAt = asString(firstEntry.timestamp) ?? asString(lastEntry.timestamp) ?? mtimeIso();
