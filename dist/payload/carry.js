...
const IN_PROGRESS_VALUES = Object.freeze([
 ...
 */
export function normalizeCarryMeta(raw) {
    ...
    unreadable.sort((a, b) => Object.keys(meta).indexOf(a) - Object.keys(meta).indexOf(b));
    return { meta: unreadable.length === 0 ? raw : meta, unreadable };
}
/**
...
function collectFiles(projectPath, rels, rules, diagnostics) {
    ...
    for (const { rel, reIncluded } of rels) {
        ...
        if (!st.isFile()) {
            ...
            // refused".
            if (reIncluded) {
                diagnostics.push(`${JSON.stringify(rel)} is named by .sesh-mover-include but is a ${describeNonFile(st)}, not a file, so it was NOT carried. Contents are never followed through a link — that would put the link target's bytes in the bundle. If you meant to carry what it points at, name that path instead.`);
            }
 ...
 */
function renameCopySources(body, destinations) {
    const found = new Set();
    for (const dest of destinations) {
        ...
        for (let i = 0; i < dest.length; i++) {
            ...
            if (body.length < open.length + close.length)
                continue;
 ...
 */
const SUMMARY_ACCOUNTED_FOR = Object.freeze([
 ...
 */
function parseApplyPaths(stdout) {
    ...
    for (const line of summary.split("\n")) {
        ...
        if (renameCopy !== null) {
            ...
            if (readings.length === 1)
                sources.push(readings[0]);
            else
                unreadable.push(line);
            continue;
        }
        if (!SUMMARY_ACCOUNTED_FOR.some((shape) => shape.test(line)))
            unreadable.push(line);
    }
...
export async function applyCarry(opts) {
    ...
    if (patchBytes > 0 && targetExists) {
        const numstat = git(targetPath, applyInvocation(applyPrefix, ["--numstat", "-z", "--summary"], patchPath), LS_FILES_MAX_BUFFER);
        if (numstat.ok) {
            ...
            if (described.unreadable.length > 0) {
                ...
                return decline("unsafe-payload", `git's own description of this patch could not be read back, so the paths it writes could not all be checked (${described.unreadable.slice(0, 2).map((l) => JSON.stringify(l)).join(", ")})`);
            }
    ...
    if (headSha !== meta.baseCommit) {
        return decline("wrong-base", `this machine is at commit ${headSha.slice(0, 8)}, the changes were captured at ${orNotRecorded(meta.baseCommit.slice(0, 8))} on branch ${orNotRecorded(meta.branch)}`);
    }
    ...
    if (patchBytes > 0) {
        ...
        if (!gitFloorAnswered) {
            return decline("apply-failed", `git could not describe the carried patch here, so the paths it writes could not be checked${gitFloorDetail ? ` (${gitFloorDetail})` : ""}`, 
