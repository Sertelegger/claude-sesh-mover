...
function migrateLegacySyncStateFile(newPath: string, projectPath: string): void {
  if (existsSync(newPath)) return;
  const legacy = legacyEncodeProjectPath(projectPath);
  if (legacy === encodeProjectPath(projectPath)) return;
...
export function forgetSentToPeer(
  state: SyncState,
  peer: { id: string },
  opts?: { localSessionIds?: string[]; memoryDigest?: boolean }
): ForgetSentResult {
  ...
  if (scope) {
    ...
    if (dropMemory) delete entry.memoryDigest;
