...
import {
  applyCarry, normalizeCarryMeta, orNotRecorded,
  type ApplyResult, type CarryMeta,
 ...
 */
function layerDirsFor(
  exportPath: string,
  bundleSessionId: string
): Array<["subagents" | "tool-results" | "file-history", string]> {
  return [
...
interface SharedLayerReport {
  ...
  plansSkipped?: number;
  /** Memory files the bundle carried that `--no-memory` declined. */
  memorySkipped?: number;
  /** Every path outside a minted session id this run writes (#36). */
  writeSet: WriteSet;
}
 ...
 */
function sharedFindings(shared: SharedLayerReport): SharedLayerFindings {
  return {
    memoryConflicts:
 ...
 */
function layerRootStatus(path: string): "absent" | "symlink" | "present" {
  try {
...
function reconcileSharedLayers(opts: {
  ...
  /**
   * Decline the bundle's `memory/`. **On unless the caller says otherwise** —
   * the exact opposite default from `includePlans`, and the asymmetry is the
   * recorded decision (#36). A memory lands in the target PROJECT's own
   * directory, is add-only, and parks a conflict rather than overwriting it, so
   * its blast radius is the one directory the user is already working in. It is
   * also the layer a future session reads prose out of, which is why making it
   * opt-in would silently break the primary single-owner flow ("I exported to
   * my laptop and my memories came with"). What was missing was not a gate but
   * an off switch the CLI honors: before this, a direct `sesh-mover import`
   * applied the memory layer with no way to decline it, and the only consent
   * was prose in a markdown file the model may skip.
   *
   * NEGATIVE, not `includeMemory`, and that is a safety property rather than a
   * style one: a field whose ABSENCE means "on" invites a caller to copy the
   * `includePlans === true` line next to it, which would silently stop the
   * memory layer for every caller that does not set it. Written this way, the
   * copy-paste (`opts.noMemory === true`) is correct. Same shape and same
   * reason as `ImportOptions.noRegister`.
   */
  noMemory?: boolean;
   ...
   */
  const writeEntries: WriteSetEntry[] = [];
  const writeRoots: WriteSetRoot[] = [];
  const recordWrite = (
    layer: WriteSetLayer,
    path: string,
    kind: WriteSetEntry["kind"]
  ): void => {
  ...
  if (memoryRoot === "symlink") {
    warnings.push(
      `The memory folder in this bundle is a symlink, not a directory — nothing was read through it, and nothing in your memory folder was changed.`
    );
  }
  ...
  if (memoryRoot === "present" && !includeMemory) {
    ...
    try {
      ...
      memorySkipped = readdirSync(memoryDir).filter((f) =>
    ...
    if (memorySkipped !== undefined && memorySkipped > 0) {
      warnings.push(
        `This bundle carries ${memorySkipped} memory file(s), and they were NOT written, because \`sesh-mover import --no-memory\` was passed. Every one of them is still in the bundle, so nothing was consumed by declining: re-run the same import without that flag to land them.`
      );
    }
  }
  if (memoryRoot === "present" && includeMemory) {
    try {
      ...
      if (indexExistedBefore) readLocalIndex();
      ...
      for (const file of files) {
        ...
        if (!reusedParked) recordWrite("memory", join(targetMemDir, parkedAs), "park");
      ...
      if (indexNeedsWrite) indexWillBeWritten = true;
 ...
 */
function emptyPayloadReport(): PayloadLayerReport {
  return {
...
/** Everything the two file-payload layers produced, for one import. */
interface PayloadLayerReport {
  warnings: string[];
  writeEntries: WriteSetEntry[];
  writeRoots: WriteSetRoot[];
  /** Paths the workspace payload names, uncapped — counted whether or not applied. */
  workspacePathCount: number;
  /**
   * Of those, how many this run WILL WRITE. `0` when the layer was declined,
   * which is what keeps `WriteSet.total` a promise about writes rather than a
...
async function reconcilePayloadLayers(opts: {
  ...
  /** Carry meta the manifest declared, if any. */
  declaredCarry?: CarryMeta;
  /** The manifest declares a workspace payload. */
  declaresWorkspace: boolean;
  plan: boolean;
}): Promise<PayloadLayerReport> {
  const {
    exportPath, targetProjectPath, applyWorkspace, forceWorkspace, plan,
  } = opts;
  const warnings: string[] = [];
  const writeEntries: WriteSetEntry[] = [];
  const writeRoots: WriteSetRoot[] = [];
  const report: PayloadLayerReport = {
    ...
    try {
      const ws = await unpackWorkspace(incomingDir, targetProjectPath, {
        ...
        onFile: (relPath, existing) => {
          ...
          if (!applyWorkspace) return;
          report.workspaceWriteCount++;
          if (writeEntries.length >= MAX_WORKSPACE_WRITE_ENTRIES) return;
      ...
      if (applyWorkspace && !plan) {
        ...
        const missingRules = [INCLUDE_FILE_NAME, IGNORE_FILE_NAME].filter(
          (name) => !existsSync(join(targetProjectPath, name))
        );
        if (missingRules.length > 0) {
          warnings.push(
            `The workspace payload could not carry ${missingRules.join(" or ")} — those names are on the floor that stops a bundle rewriting what this machine's next push uploads, so no payload may contain them. This project therefore has ${missingRules.length > 1 ? "neither file" : "no such file"} and will carry a different set of files than the source did. Copy ${missingRules.length > 1 ? "them" : "it"} across by hand if the source had ${missingRules.length > 1 ? "them" : "it"}; for a git project they are meant to be committed, so a clone already has ${missingRules.length > 1 ? "them" : "it"}.`
          );
        }
      }
      ...
      if (ws.blocked.length > 0) {
        warnings.push(
          `${ws.blocked.length} workspace file(s) were not unpacked because of what already occupies their path here (${[...new Set(ws.blocked.map((b) => b.reason))].join(", ")}): ${ws.blocked.slice(0, 5).map((b) => b.path).join(", ")}. Nothing was written near them; the incoming copies are still in the bundle.`
        );
      }
      if (!applyWorkspace) {
        ...
        if (report.workspacePathCount > 0) {
          warnings.push(
            `This bundle carries ${report.workspacePathCount} project file(s), and they were NOT written, because \`sesh-mover import --apply-workspace\` was not passed. Every one of them is still in the bundle, so nothing was consumed by declining: re-run the same import with that flag to land them.`
          );
        }
      } else if (!plan && hasRealContent) {
        warnings.push(
          "This import unpacked the workspace payload over the existing directory, overwriting any file of the same name, because --force-workspace was passed. It is a bootstrap copy, not a merge — anything of yours the payload does not contain is still here, and anything it does contain was replaced."
        );
      }
    } catch (e) {
...
function describeCarryOutcome(result: ApplyResult, meta: CarryMeta): string[] {
  const out: string[] = [];
  const origin = `branch ${orNotRecorded(meta.branch)} at commit ${orNotRecorded(meta.baseCommit.slice(0, 8))}`;
  if (!result.applied) {
    out.push(
      `The uncommitted changes this bundle carries (${origin}) were not applied: ${result.detail}. ` +
        (result.savedTo === null
    ...
    if (result.refused.length > 0) {
      out.push(
        `${result.refused.length} path(s) in that payload were left out of the saved copy because they name plugin or VCS internals that never travel (${result.refused.slice(0, 5).join(", ")}). They are not in the saved directory, so the commands in its README cannot write them here.`
      );
    }
  ...
  out.push(
    `Applied the uncommitted changes this bundle carries (${origin}): ${result.filesChanged} file(s) from the patch, ${result.untrackedCopied} untracked file(s) copied. They are uncommitted here too — \`git status\` shows them, and \`git checkout -- .\` undoes the patch half.`
  );
  if (meta.inProgress) {
    out.push(
      `Those changes were captured during an in-progress ${meta.inProgress} on the source machine, so the patch contained conflict markers as ordinary file content and the ${meta.inProgress} itself did not travel — search for <<<<<<< before working on them.`
    );
  }
  if (result.collisions.length > 0) {
    out.push(
      `${result.collisions.length} carried file(s) already existed here with different content, so yours were left alone and the incoming copies were written beside them as *.incoming-*: ${result.collisions.slice(0, 5).join(", ")}. Reconcile and delete the sidecars.`
    );
  }
  if (result.refused.length > 0) {
    out.push(
      `${result.refused.length} carried file(s) were refused because they name plugin or VCS internals that never travel (${result.refused.slice(0, 5).join(", ")}). Nothing from them was written.`
    );
  }
 ...
 */
function payloadFindings(p: PayloadLayerReport): PayloadFindings {
  return {
...
function mergeWriteSets(shared: SharedLayerReport, payload: PayloadLayerReport): WriteSet {
  return {
...
export async function importSession(
  ...
  if (digestProblem) {
    return {
      ...
      details:
        "Nothing was read from the bundle beyond its manifest, and nothing was written. The manifest's session list is not the one the export produced — the bundle was damaged in transit or edited after it was written.",
      suggestion:
        "Re-export or re-transfer the bundle. This check detects damage, not tampering: it cannot tell a corrupted bundle from one a sender rewrote deliberately.",
    };
  }
  ...
  const payloadFor = (plan: boolean): Promise<PayloadLayerReport> =>
    handlesPayload
  ...
  const absentFromBundle = targetSessions.filter(
  ...
  if (applyWorkspace && !forceWorkspace && isReadableDir(join(exportPath, "workspace"))) {
    ...
    if (occupants.some((n) => !isPluginStateName(n))) {
      return {
        ...
        details:
          "No sessions were imported and no project files were written. An export bundle's workspace payload is a bootstrap copy, not a merge: applying it here would overwrite files of the same name.",
        suggestion:
          "Re-run this import against an empty directory with --target-project-path <dir>, or pass --force-workspace to unpack over what is there, OVERWRITING any file of the same name — it does not combine the two. To take the sessions only, drop --apply-workspace.",
      };
    }
  }
  ...
  try {
    for (const [sessionIndex, session] of targetSessions.entries()) {
      ...
      const layerOk = (layer: "subagents" | "tool-results" | "file-history"): boolean =>
        !failedLayers.has(`${session.sessionId}\0${layer}`);
