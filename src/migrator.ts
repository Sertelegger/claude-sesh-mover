...
export async function migrateSession(
  ...
  try {
    ...
    const keptUncarried = new Set<string>();
    ...
    for (const movedId of movedIds) {
      ...
      if (existsSync(sessionSubDir)) {
        ...
        if (readdirSync(sessionSubDir).length === 0) rmSync(sessionSubDir, { recursive: true });
      }
