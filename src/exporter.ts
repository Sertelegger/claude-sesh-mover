 ...
 */
async function computeSessionLayerDigests(
  exportPath: string,
  manifestSessionId: string
): Promise<SessionManifest["layerDigests"]> {
  const dirs = {
  ...
  const out: NonNullable<SessionManifest["layerDigests"]> = {};
  for (const [layer, dir] of Object.entries(dirs) as Array<
    [keyof typeof dirs, string]
  >) {
    const digest = await computeLayerDigest(dir);
    if (digest !== null) out[layer] = digest;
  }
 ...
 */
function copyDirIfExists(srcDir: string, destDir: string): boolean {
...
export interface ExportOptions {
   ...
   */
  payloadBudgets?: { workspaceMaxBytes: number; carryMaxBytes: number };
...
async function exportSessions(
  ...
  const {
    configDir, projectPath, excludeLayers, claudeVersion,
    summaryOverrides, noSummary, incremental, onProgress,
  } = options;
  ...
  const landedLayers = new Set<ExportLayer>();
  ...
  const landedBySession = new Map<string, Set<ExportLayer>>();
  const record = (manifestSessionId: string, layers: Set<ExportLayer>): void => {
    landedBySession.set(manifestSessionId, layers);
    for (const l of layers) landedLayers.add(l);
  };
  const layerList = (set: Set<ExportLayer>): ExportLayer[] =>
    getAllLayers().filter((l) => set.has(l));
  ...
  for (const [sessionIndex, session] of toFull.entries()) {
    ...
    const sessionLayers = new Set<ExportLayer>(["jsonl"]);
    if (
      requestedLayers.includes("subagents") &&
    ...
    if (
      requestedLayers.includes("tool-results") &&
    ...
    if (
      requestedLayers.includes("file-history") &&
  ...
  for (const [contIndex, item] of toContinuation.entries()) {
    ...
    const contLayers = new Set<ExportLayer>(["jsonl"]);
    if (
      requestedLayers.includes("subagents") &&
  ...
  // for `memory`.
  const bundleLayers = new Set<ExportLayer>();
  ...
  let workspaceMeta: ExportManifest["workspace"] | undefined;
  ...
  for (const l of bundleLayers) landedLayers.add(l);
  ...
  return {
    ...
    sessions: sessionManifests.map((s) => ({
      ...
      exportedLayers: layerList(
        new Set([...(landedBySession.get(s.sessionId) ?? []), ...bundleLayers])
      ),
    })),
    warnings,
    archivePath: null,
