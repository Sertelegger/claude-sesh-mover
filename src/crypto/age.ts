...
/**
 * How far the search for the end of the header got.
 *
 * Bounded and linear, and both properties are about the input rather than about
 * tidiness: these bytes came off the hub, so a file that never contains a mark
 * must not be accumulated in full (`MAX_HEADER_BYTES`) and must not be rescanned
 * from the start on every write (`scannedTo`).
 *
 * ONE COPY, shared by the decryptor and the re-wrapper — a second scan would be
 * a second place for those two bounds to be got wrong, on the side that reads
 * hub-supplied bytes.
 */
type HeaderScan = { done: false; scannedTo: number } | { done: true; macLineEnd: number };

function scanForHeaderEnd(pending: Buffer, scannedTo: number): HeaderScan {
 ...
 * verbatim begin.
 */
function openHeader(header: Buffer, identityRaw: Buffer): { fileKey: Buffer; parsed: ParsedHeader } {
...
export class AgeRewrapStream extends Transform {
  ...
  private scannedTo = 0;
  private headerDone = false;
  private stanzasSeen: number | null = null;
  private payloadBytes = 0;
  private readonly identityRaw: Buffer;
  private readonly recipients: Buffer[];
