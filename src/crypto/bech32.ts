...
export function bech32Decode(s: string): { hrp: string; data: Uint8Array } {
  ...
  if (hasUpper && hasLower) throw new Error("bech32: mixed case");
  ...
  if (sep < 1 || sep + 7 > norm.length) throw new Error("bech32: bad separator position");
  const hrp = norm.slice(0, sep);
  const words: number[] = [];
  for (const ch of norm.slice(sep + 1)) {
    const idx = CHARSET.indexOf(ch);
    if (idx === -1) throw new Error(`bech32: invalid character ${ch}`);
    words.push(idx);
  }
