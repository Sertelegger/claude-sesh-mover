...
export type SigningKeyState =
  | { state: "absent" }
  | { state: "unreadable"; cause: SigningKeyCause; detail: string }
  | {
      state: "present";
...
function hasInsecureMode(p: string): boolean {
 ...
 */
export function publicKeyToBase64Url(key: KeyObject): string {
 ...
 */
export function readSigningKey(): SigningKeyState {
...
export type SigningKeyLoad =
  | { ok: true; privateKey: KeyObject; publicKey: string; created: boolean; insecureMode: boolean }
  | { ok: false; cause: SigningKeyCause; detail: string };
 ...
 */
export function loadOrCreateSigningKey(): SigningKeyLoad {
 ...
 */
export function verifyBytes(publicKeyBase64Url: string, data: Buffer, signature: string): boolean {
  try {
