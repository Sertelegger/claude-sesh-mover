...
program
  ...
  .option(
    "--include-workspace",
    "Also capture a copy of the project's working tree (only for a project with NO git remote; does not read .gitignore)"
  )
  .option(
    "--include-carry",
    "Also capture uncommitted work as a git patch plus untracked files (only for a project WITH a git remote)"
  )
  .option(
    "--payload-plan",
    "Measure what --include-workspace / --include-carry would carry and report it, writing nothing. Exports no sessions."
  )
...
program
  ...
  .option(
    "--no-memory",
    "Do not write the bundle's memory/ into this project's memory folder (written by default — it lands in the target project's own directory)"
  )
  // ---------------------------------------------------------------------
  ...
  .option(
    "--apply-workspace",
    "Unpack the bundle's workspace/ into the target project directory (refused if it is not empty, unless --force-workspace)"
  )
  .option(
    "--apply-carry",
    "Apply the bundle's carried uncommitted changes (requires the same base commit and a clean tree)"
  )
  .option(
    "--force-workspace",
    "Unpack the workspace payload over a target that already has content, OVERWRITING any file of the same name (it does not combine the two)"
  )
  .option("--progress", "Emit NDJSON progress events on stderr")
  .action(async (opts) => {
    ...
    try {
      ...
      if (archiveFormat) {
        ...
        const result: EncryptedBundleRefusedResult = {
          success: false,
          command: "import",
          reason: "encrypted-bundle",
          error:
            `"${fromPath}" is an age-encrypted bundle — import reads plaintext ` +
            `archives (.tar.gz/.tgz, .tar.zst/.tar.zstd) and export directories only`,
          bundlePath: fromPath,
          decryptedPath,
          suggestion:
            `The bundle is not damaged; it is encrypted. Decrypt it with the standard age tool, ` +
      ...
      if (opts.set) {
        ...
        if (configValueKind(key) === "number") {
          ...
          try {
            ...
            outputError(
              "configure",
              new Error(`Invalid JSON for ${key}: ${errorMessage(parseErr)}`)
            );
            return;
          }
        }
...
hub
  ...
  .action(async (opts) => {
    try {
      ...
      outputError("hub-unlink", e as Error);
    }
  });
...
hub
  ...
  .action(async (opts) => {
    try {
      ...
      outputError("hub-retire", e as Error);
    }
  });
...
hub
  ...
  .action(async (opts) => {
    try {
      ...
      outputError("hub-delete", e as Error);
    }
  });
...
hub
  ...
  .action(async (opts) => {
    try {
      ...
      outputError("hub-encrypt", e as Error);
    }
  });
...
hub
  ...
  .option(
    "--unsigned",
    "Rebuild without re-signing. Use when you suspect the hub's bundles were altered — a rebuild signs whatever is there now"
  )
  .action(async (opts) => {
    try {
      const configDir = resolveConfigDir(opts.sourceConfigDir);
...
program
  ...
  .option(
    "--force-workspace",
    "Unpack the hub's workspace copy over a non-empty target directory, overwriting files of the same name (never a merge)"
  )
  ...
  .option(
    "--ignore-retirement",
    "Pull even though the project has been retired on the hub (its bundles may be deleted soon)"
  )
  .option("--source-config-dir <path>", "Override Claude config dir")
  .option("--progress", "Emit NDJSON progress events on stderr")
  .action(async (opts) => {
...
async function doExport(
  options: import("./exporter.js").ExportOptions & { scope: SessionScope }
) {
...
async function finalizeExport(params: {
  ...
  if (format === "archive" || format === "zstd") {
    ...
    try {
      ...
      if (!(e instanceof ZstdNoContentChecksumError)) throw e;
      result.warnings.push(
        `${e.message} — falling back to gzip, whose CRC32 does detect it.`
      );
      compression = "gzip";
 ...
 */
function recordAutoPushOutcome(projectPath: string, result: { success: boolean }): void {
  try {
    ...
    if (result.success) {
      ...
      if (r.orphanBundle === true) {
        notes.push(
          "A bundle reached the hub but this machine's index was not updated to reference it, so no other machine can see it yet; the next successful push republishes the index."
        );
      }
    }
 ...
 */
function outputError(command: string, error: unknown): void {
