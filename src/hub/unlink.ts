...
export function hubUnlink(
  ...
  if (opts.force) {
    ...
    if (lock.stoleStale) {
      warnings.push(
        "Stole a stale project lock left by a previous sesh-mover hub operation (likely crashed or was killed) — the unlink proceeded, but verify no push or pull is genuinely in progress."
      );
    }
  }
