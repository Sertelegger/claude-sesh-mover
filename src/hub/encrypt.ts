  ...
  if (opts.enable) {
    ...
    try {
      ...
      preferenceWarnings.push(
        `The hub-wide setting was written, but this machine's local hub.encrypt preference could not be recorded (${errorMessage(e)}). That affects nothing but what \`configure --show\` reports here.`
      );
    }
  }
