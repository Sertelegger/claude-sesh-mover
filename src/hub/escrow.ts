...
export function escrowRecordPath(): string {
  return join(userSeshMoverDir(), ESCROW_RECORD_FILE_NAME);
}

interface EscrowRecordJson {
  schemaVersion: 1;
  /** Absolute, as resolved at enable time. */
  path: string;
  /** `age1…` — the PUBLIC half the escrowed identity corresponds to. */
  recipient: string;
  createdAt: string;
  workFactorLogN: number;
}
...
export type DestinationVerdict =
  ...
  | { ok: false; rule: UnsafeOutRule; path: string; detail: string; suggestion: string };
 ...
 */
const SYNCED_DIR_NAMES: readonly string[] = [
  "dropbox",
  "google drive",
  "googledrive",
  "my drive",
...
const SYNCED_MARKER_FILES: readonly string[] = [
  ".dropbox",
  ".dropbox.cache",
  ".stfolder",
  ".stversions",
 ...
 */
export const ESCROW_DESTINATION_LIMITS: readonly string[] = [
 ...
 */
export function checkEscrowDestination(
  outPath: string,
  ctx: { cwd: string; hubPath: string | null }
): DestinationVerdict {
  ...
  if (!existsSync(parent) || !statSync(parent).isDirectory()) {
    return {
      ...
      suggestion:
        "Create the directory yourself and run this again. This command does not create " +
  ...
  if (existsSync(path)) {
    return {
      ...
      suggestion:
        "Choose a path that does not exist. Nothing is overwritten here: the file already " +
  ...
  if (hubReal && isInside(dir, hubReal)) {
    return {
      ...
      suggestion:
        "Put the escrow somewhere the hub does not reach. Writing it here escrows the key into " +
  ...
  if (isInside(dir, cwdReal) || projectDir) {
    return {
      ...
      suggestion:
        "Put the escrow outside every project. A project's tree is what `push` uploads to the " +
  ...
  if (gitDir) {
    return {
      ...
      suggestion:
        "Put the escrow outside every repository. One `git add -A` and one push and the escrow " +
  ...
  const synced = chain.find(
    (d) =>
      SYNCED_DIR_NAMES.includes(basename(d).toLowerCase()) ||
  ...
  if (synced) {
    return {
      ...
      suggestion:
        "Put the escrow somewhere that is not uploaded anywhere. A synced folder hands the file " +
...
function readRecord(): EscrowRecordJson | null {
  const p = escrowRecordPath();
  if (!existsSync(p)) return null;
  try {
    const parsed = JSON.parse(readFileSync(p, "utf-8")) as unknown;
    if (!parsed || typeof parsed !== "object") return null;
    const r = parsed as Partial<EscrowRecordJson>;
    if (typeof r.path !== "string" || typeof r.recipient !== "string") return null;
    return {
      schemaVersion: 1,
      path: r.path,
...
function writeRecord(r: EscrowRecordJson): void {
 ...
 * else's.
 */
function inspectEscrowFile(p: string): { present: boolean; looksLikeEscrow: boolean; logN: number | null } {
  if (!existsSync(p)) return { present: false, looksLikeEscrow: false, logN: null };
  let head = "";
  try {
  ...
  if (!m) return { present: true, looksLikeEscrow: false, logN: null };
 ...
 */
export function escrowRecoverySteps(escrowPath: string): string[] {
  return [
...
function refuse(
  ...
  error: string,
  suggestion: string,
  extra: Partial<HubEscrowRefusedResult> = {}
 ...
 */
function enableWarnings(input: { passphraseBytes: number; insecureMode: boolean }): string[] {
  const w = [
  ...
  if (input.passphraseBytes < 12) {
    w.push(
      "That passphrase is short. The work factor slows a guess by a fixed amount; it does not make a " +
        "short passphrase hard. Everything above is only as strong as what you typed."
    );
  }
  if (input.insecureMode) {
    w.push(
      "The identity file this escrow copies is readable by group or other on this machine. Fix its " +
...
export async function hubEscrow(
  opts: HubEscrowOptions
): Promise<HubEscrowResult | HubEscrowRefusedResult> {
  ...
  if (opts.action === "status") {
    ...
    if (record && file && !file.present) {
      warnings.push(
        "The escrow file recorded here is not at that path any more. Either you moved it — which is " +
    ...
    if (record && file?.present && !file.looksLikeEscrow) {
      warnings.push(
        "There is a file at the recorded path, but it does not begin like a passphrase-addressed age " +
    ...
    if (record && identity.state === "present" && identity.recipient !== record.recipient) {
      warnings.push(
        "This escrow is for a key this machine no longer has. Restoring it would restore the OLD " +
    ...
    return {
      ...
      current:
        record === null || identity.state !== "present"
          ? null
  ...
  if (opts.action === "disable") {
    if (!record) {
      return refuse(
        "not-enabled",
        "No escrow is recorded on this machine.",
        "There is nothing to turn off. `hub escrow` with no flags reports the current state."
      );
    }
    try {
  ...
  if (opts.passphrase.kind === "not-requested") {
    return refuse(
      "passphrase",
      "Enabling the escrow needs a passphrase, and the only way to supply one is --passphrase-stdin.",
      "Run this in YOUR OWN SHELL, never in a chat session:\n" +
  ...
  if (opts.passphrase.kind === "terminal") {
    return refuse(
      "passphrase",
      "Standard input is a terminal, so reading the passphrase would echo it.",
      "Pipe it instead:\n" +
        "  read -rs SESH_ESCROW && printf '%s' \"$SESH_ESCROW\" | " +
        "sesh-mover hub escrow --enable --passphrase-stdin --out <path>"
    );
  ...
  if (passphrase.length === 0) {
    return refuse(
      ...
      "An empty passphrase is not a weak escrow, it is a public one: every age implementation " +
  ...
  if (passphrase.includes(0x0a) || passphrase.includes(0x0d)) {
    return refuse(
      "passphrase",
      "The passphrase contains a line break.",
      "age reads a passphrase as a single line from the terminal, so a passphrase with a line " +
  ...
  if (!opts.outPath) {
    return refuse(
      "no-out",
      "--out is required: the escrow is written to a path you name.",
      "There is no default. A default would put a passphrase-wrapped private key somewhere the " +
  ...
  if (identity.state !== "present") {
    return refuse(
      ...
      identity.state === "absent"
        ...
        : `This machine's identity file cannot be read (${identity.cause}): ${identity.detail}`,
      identity.state === "absent"
  ...
  try {
    await pipeline(
      Readable.from([Buffer.from(identity.raw, "utf-8")]),
      new AgeEncryptStream({ passphrase, logN }),
  ...
  try {
    const chunks: Buffer[] = [];
    await pipeline(
      Readable.from([readFileSync(dest.path)]),
      new AgeDecryptStream({ passphrase }),
