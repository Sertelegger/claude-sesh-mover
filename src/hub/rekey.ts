...
export async function hubRekey(
  ...
  try {
    ...
    const skipped: HubRekeyResult["skipped"] = [];
    const failed: HubRekeyResult["failed"] = [];
    const narrowed: HubRekeyResult["narrowed"] = [];
    ...
    for (const file of files) {
      ...
      rewrapped.push(file);
      if (
        out.previousRecipientStanzas !== null &&
    ...
    if (narrowed.length > 0) {
      warnings.push(
        `${narrowed.length} file(s) came out addressed to FEWER machines than they went in. A re-wrap addresses a file to the hub's roster as it stands, and a recipient stanza carries an ephemeral share rather than a public key, so which machines were dropped cannot be recovered from the file — only that some were. The usual cause is a machines/<id>.json that has been removed or damaged since the file was written. Fixing the roster and running this again re-includes them.`
      );
    }
