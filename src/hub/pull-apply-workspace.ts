...
async function retrieveWorkspaceTree(
  ...
  | { dir: string; digest: string }
  | { dir: null; kind: "unavailable"; why: string }
  | { dir: null; kind: "digest-mismatch"; expected: string; found: string }
> {
  try {
...
async function fetchAncestorWorkspace(
  backend: HubBackend,
  ref: { bundleId: string; file: string; digest?: string },
  tempRoot: string
...
export async function runApplyWorkspaceStage(
  ...
  if (incoming.kind === "inline" && bundleDeclaresWorkspace && !isReadableDir(inlineDir)) {
     ...
     * deciding anything.
     */
    const materializeIncoming = async (): Promise<
      { dir: string; digest?: string } | { dir: null; unverified?: true }
    > => {
       ...
       */
      const att = input.workspaceAttestation ?? { kind: "unsigned" as const };
    ...
    if (ancestorDir !== null) {
      ...
      if (inc.dir === null) {
        ...
        return stageSkip<WorkspaceStageValue>(
          reasons,
          inc.unverified ? { unpacked: null, unverified: true } : { unpacked: null, declaredMissing: true }
        );
      }
