...
export async function registerMachine(hubPath: string): Promise<HubMachineJson> {
  ...
  let ageRecipient: string | undefined = key.ok ? key.recipient : undefined;
  const signing = loadOrCreateSigningKey();
  let signingPublicKey: string | undefined = signing.ok ? signing.publicKey : undefined;
  if (ageRecipient === undefined || signingPublicKey === undefined) {
    try {
      const prior = JSON.parse(
        (await backend.read(machinePath(identity.id))).toString()
      ) as HubMachineJson;
