 ...
 */
export const NEVER_INCLUDABLE: readonly string[] = [".git", ".claude-sesh-mover"];
 ...
 */
export function readHubinclude(projectPath: string): string[] {
  ...
  if (!st.isFile() || st.size > MAX_HUBINCLUDE_BYTES) return [];
  return readFileSync(p, "utf-8")
    .split("\n")
    .map((l) => l.trim())
    .filter((l) => l.length > 0 && !l.startsWith("#"))
 ...
 * stricter.
 */
function toSegments(input: string, kind: "path" | "pattern"): string[] | null {
 ...
 */
const patternCache = new WeakMap<readonly string[], string[][]>();

function parsePatterns(patterns: readonly string[]): string[][] {
  ...
  for (const raw of patterns) {
    ...
    if (segments.some(isNeverSegment)) continue;
    const normalized: string[] = [];
    let tooWild = false;
    for (const segment of segments) {
      const collapsed = segment.replace(/\*+/g, "*");
      if ((collapsed.match(/\*/g)?.length ?? 0) > MAX_PATTERN_WILDCARDS) { tooWild = true; break; }
      normalized.push(collapsed);
    }
    if (!tooWild) parsed.push(normalized);
  }
...
export function isReIncluded(relPath: string, patterns: string[]): boolean {
  const segments = toSegments(relPath, "path");
  if (segments === null || segments.some(isNeverSegment)) return false;
...
export function mayContainReIncluded(dirRelPath: string, patterns: string[]): boolean {
  ...
  if (segments === null || segments.some(isNeverSegment)) return false;
  for (const pattern of parsePatterns(patterns)) {
    if (pattern.length === 1) return true; // bare: can match at any depth below
    if (
      pattern.length > segments.length &&
...
export async function snapshotWorkspace(
  projectPath: string,
  destDir: string
): Promise<{ fileCount: number; byteSize: number; symlinksSkipped: number }> {
  ...
  const walk = (srcDir: string, outDir: string, rel: string, insideExcluded: boolean): void => {
    for (const entry of readdirSync(srcDir, { withFileTypes: true })) {
      ...
      if (isNeverSegment(entry.name)) continue;
      ...
      if (!carried) {
        ...
        if (!entry.isDirectory() || !mayContainReIncluded(childRel, includePatterns)) continue;
      }
