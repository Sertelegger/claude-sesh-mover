...
import {
  readSyncState, writeSyncState, setThreadId, setLastWorkspace, recordSentToPeer,
} from "../sync-state.js";
import type {
  ErrorResult,
  ExportManifest,
...
async function fetchAncestorWorkspace(
  backend: HubBackend,
  ref: { bundleId: string; file: string } | undefined,
  tempRoot: string
): Promise<{ dir: string | null; warning?: string }> {
  if (!ref) return { dir: null };
...
interface GenerationRef { bundleId: string; file: string; pushedAt?: string }
...
async function chooseMergeAncestor(
  backend: HubBackend,
  incomingBase: GenerationRef | null | undefined,
  localRef: GenerationRef | undefined,
  tempRoot: string
): Promise<{ dir: string | null; warnings: string[] }> {
  const warnings: string[] = [];
  const ordered: GenerationRef[] =
    incomingBase && localRef && incomingBase.pushedAt && localRef.pushedAt
      ? (incomingBase.pushedAt < localRef.pushedAt
        ? [incomingBase, localRef]
        : [localRef, incomingBase])
      : [localRef, incomingBase].filter((r): r is GenerationRef => !!r);
  ...
  for (const ref of ordered) {
    ...
    if (attempt.dir !== null) return { dir: attempt.dir, warnings };
...
function describeWorkspaceMerge(r: WorkspaceMergeReport): string[] {
  ...
  if (r.conflicted.length > 0) {
    out.push(
      `${count(r.conflicted.length)} were merged with conflict markers and need resolving by hand — search for "<<<<<<< local": ${names(r.conflicted)}. Conflicts are normal here: edits on adjacent lines conflict, and a file added independently on both machines conflicts over its whole length.`
    );
  }
  if (r.gitUnavailable) {
    out.push(
  ...
  if (r.sidecars.length > 0) {
    out.push(
      `${count(r.sidecars.length)} could not be merged, so your copies were kept and the other machine's were saved alongside as *.theirs-* files: ${names(r.sidecars.map((s) => s.path))}. Delete the sidecars once you've reconciled them — they are ordinary files and will be pushed to the hub otherwise.`
    );
  }
  if (r.skipped.length > 0) {
    out.push(
      `${count(r.skipped.length)} were not applied at all and nothing was written near them (${[...new Set(r.skipped.map((s) => s.reason))].join(", ")}): ${names(r.skipped.map((s) => s.path))}. The incoming copies are still on the hub.`
    );
  }
  if (r.upstreamDeleted.length > 0) {
    out.push(
      `${count(r.upstreamDeleted.length)} were deleted on the other machine but kept here (this merge never deletes your files): ${names(r.upstreamDeleted)}.`
    );
  }
  if (r.localDeleted.length > 0) {
    out.push(
      `${count(r.localDeleted.length)} that you deleted here since the last sync were not restored: ${names(r.localDeleted)}. They are unchanged on the other machine, so they are still on the hub if you want them back.`
    );
  }
  if (r.restored.length > 0) {
    out.push(
      `${count(r.restored.length)} that you had deleted here were changed on the other machine, so they came back rather than losing that work: ${names(r.restored)}. Delete them again if you still don't want them.`
    );
  }
  return out;
}
      ...
      if (i === workspaceBundleIndex && bundleManifest.workspace) {
        ...
        if (hasRealContent) {
          const ancestor = await chooseMergeAncestor(
            backend,
            bundleManifest.workspace.basedOn,
            readSyncState(effectiveProjectPath).hub?.lastWorkspace,
            tempRoot
          );
        ...
        if (ancestorDir !== null) {
          ...
          warnings.push(
            "Bundle carries a workspace payload but the project directory already has content and this machine has no workspace generation recorded to merge against — pass --force-workspace to unpack it here, OVERWRITING any file of the same name, or re-pull with --target-path <fresh-dir> to unpack it elsewhere. Once this machine has pushed or applied one generation, later payloads merge 3-way instead."
          );
        } else {
          try {
            ...
            if (hasRealContent) {
              warnings.push(
                "The workspace payload was unpacked over the existing directory, overwriting any file of the same name — there was no recorded workspace generation to merge against, so nothing could be combined."
              );
            }
