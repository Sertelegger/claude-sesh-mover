 ...
 */
export async function listMachineIds(backend: HubBackend): Promise<string[]> {
...
export type MachineRecordRead =
  | { ok: true; record: HubMachineJson }
  | { ok: false; problem: MachineRecordProblem };
 ...
 */
export async function readMachineRecord(
  backend: HubBackend,
  machineId: string
): Promise<MachineRecordRead> {
  if (!isSafeSessionId(machineId)) return { ok: false, problem: "unsafe-id" };
  let raw: Buffer;
  try {
  ...
  try {
    const parsed = JSON.parse(raw.toString()) as HubMachineJson;
