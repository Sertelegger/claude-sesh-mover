 ...
 */
export function planAck(input: AckPlanInput): AckPlan {
  ...
  const ackHeads = new Set<string>([input.consolidatedHead]);
  for (;;) {
    let grew = false;
    for (const r of surviving) {
      if (!isLinkUuid(r.headEntryUuid) || ackHeads.has(r.headEntryUuid)) continue;
  ...
  const outstanding: OutstandingMachine[] = [];
  ...
  for (const index of input.indexes) {
    ...
    const anchored = entry.bundles.some(
      (r) => isLinkUuid(r.anchorEntryUuid) && ackHeads.has(r.anchorEntryUuid)
    );
    if (anchored) {
      acknowledged.push(index.machineId);
      continue;
    ...
    const reason: OutstandingReason = !isLinkUuid(entry.headEntryUuid)
      ? "unknown-head"
 ...
 */
export function planRetirement(input: RetirementPlanInput): RetirementPlan {
  const deletable: HubBundleRecord[] = [];
  const retained: RetainedBundle[] = [];
