...
function keyOf(c: AuxiliaryConflict & { parkedAs?: string }): string {
...
export function recordSharedLayers(
  acc: SharedLayerAccumulator,
  found: SharedLayerFindings
): void {
  if (found.memoryDir !== undefined) acc.memoryDir = found.memoryDir;
  ...
  for (const target of idx.added) {
    if (!acc.addedIndexEntries.includes(target)) acc.addedIndexEntries.push(target);
  }
  for (const f of idx.unindexed) {
    if (!acc.unindexed.includes(f)) acc.unindexed.push(f);
  }
 ...
 */
export function sharedLayerFindings(acc: SharedLayerAccumulator): SharedLayerFindings {
 ...
 */
export function isCarrySuppressed(
  st: Pick<ApplyState, "divergenceAborted" | "lastCarry" | "abortIndex">
): boolean {
...
export function initApplyState(input: InitApplyStateInput): ApplyState {
  ...
  const chainWorkspaceBases: Array<ChainWorkspaceBase> = [];
