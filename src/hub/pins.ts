...
const EMPTY: PinFile = { schemaVersion: 1, pins: [] };
...
export function readPins(): PinFile {
  const p = pinFilePath();
  if (!existsSync(p)) return { ...EMPTY, pins: [] };
  try {
    const parsed = JSON.parse(readFileSync(p, "utf-8")) as PinFile;
    if (!Array.isArray(parsed?.pins)) return { ...EMPTY, pins: [] };
    return {
      schemaVersion: 1,
      pins: parsed.pins.filter(
        (x): x is KeyPin =>
          typeof x?.hubId === "string" &&
...
export function findPin(pins: PinFile, hubId: string, machineId: string): KeyPin | null {
  return pins.pins.find((p) => p.hubId === hubId && p.machineId === machineId) ?? null;
}

function writePins(file: PinFile): void {
...
export type PinOutcome =
  | { kind: "unchanged"; pin: KeyPin }
  | { kind: "pinned"; pin: KeyPin }
  /** A different key than pinned. NOT written — the caller decides. */
  | { kind: "conflict"; pinned: KeyPin; found: string }
...
export function recordPin(args: {
  ...
  if (existing && existing.publicKey === publicKey) {
    if (origin === "confirmed" && existing.origin !== "confirmed") {
      const upgraded: KeyPin = { ...existing, origin: "confirmed" };
