import {
  mkdtempSync, mkdirSync, renameSync, rmSync, readFileSync, writeFileSync, existsSync,
  createReadStream,
} from "node:fs";
...
import {
  bundleDir, bundleFileName, workspaceDir, type HubBundleRecord, type HubJson,
} from "./layout.js";
import { bundleEncryptStream } from "./bundle-io.js";
import { digestFile, signStatement, SIGNATURE_DOMAIN, type BundleSignature } from "./signature.js";
 ...
 */
function unsignedPushWarning(detail: string): string {
  return (
    `This machine's signing key could not be used (${detail}), so this bundle was pushed UNSIGNED. ` +
...
export async function hubPush(opts: HubPushOptions): Promise<HubPushOutcome> {
  ...
  try {
    ...
    try {
       ...
       * take out of it.
       */
      let workspaceUpload: { staging: string; hubFile: string } | null = null;
      ...
      if (workspaceUpload) {
        workspaceDigest = await uploadArchive(
          workspaceUpload.staging,
          join(staging, "workspace.tar.gz"),
          workspaceUpload.hubFile
        );
      }
       ...
       */
      let bundleSignature: BundleSignature | undefined;
      const signingKey = loadOrCreateSigningKey();
      if (signingKey.ok) {
        try {
          bundleSignature = signStatement(
