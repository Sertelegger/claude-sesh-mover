    ...
    if (!opts.noCarry && gitRemotes().length > 0 && existsSync(opts.projectPath)) {
      ...
      if (cap.captured) {
        ...
        if (cap.meta.trackedIgnoredCount > 0) {
          ...
          warnings.push(
            `The patch carries changes to ${cap.meta.trackedIgnoredCount} gitignored file(s) that git TRACKS, so .gitignore did not keep them off the hub: ${shown}${more > 0 ? `, and ${more} more` : ""}. Untrack them (git rm --cached) or push with --no-carry to stop that.`
          );
        }
