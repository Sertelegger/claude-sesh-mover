 ...
 */
export function statementBytes(statement: BundleStatement): Buffer {
  return Buffer.from(JSON.stringify(canonicalize(statement)), "utf-8");
}

export function signStatement(privateKey: KeyObject, statement: BundleStatement, publicKey: string): BundleSignature {
...
export type SignatureFailure =
   ...
   */
  | { kind: "context-mismatch"; field: string; expected: string; found: string }
  /** The archive on disk is not the one the statement describes. */
  | { kind: "digest-mismatch"; file: string }
  /**
   ...
   */
  | { kind: "unpinned-key"; pinned: string; found: string };
...
}): { ok: true } | { ok: false; failure: SignatureFailure } {
