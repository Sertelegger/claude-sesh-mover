...
import { fetchBundleArchive, type BundleFetchFailure } from "./bundle-io.js";
 ...
 */
function retrievalSuggestion(failure: BundleFetchFailure): string {
  switch (failure.kind) {
    case "no-identity":
 ...
 */
function signatureFailureAbort(
  record: HubBundleRecord,
  machineId: string,
  failure: SignatureFailure
): StageOutcome<FetchStageResult> {
  switch (failure.kind) {
    case "bad-signature":
...
interface SignatureGateInput {
   ...
   * operation. Passed rather than read here: `tests/hub-pull-invariants.ts`
   * enforces that no pull stage reads the wall clock, so that a pull's
   * recorded times cannot disagree with each other by however long the
   * transfer took — and so a test can pin one.
   */
  nowIso: string;
  /** The local PLAINTEXT archive — already decrypted if the hub copy was sealed. */
  tarPath: string;
  /** The stage's applied-outcome sentences. The gate appends, never replaces. */
  reasons: string[];
}
 ...
 */
/**
 * Either the gate refused, or it did not and hands back what the statement said
 * about the workspace artifact. "Carry on" had to grow a payload for #110: the
 * attestation is only meaningful once the statement has been proved, and this
 * is the only place that happens.
 */
type SignatureGateOutcome =
  | { aborted: StageOutcome<FetchStageResult> }
...
async function runSignatureGate(input: SignatureGateInput): Promise<SignatureGateOutcome> {
  ...
  const signature = (record as HubBundleRecord & { signature?: BundleSignature | null }).signature;
   ...
   * exists to prevent.
   */
  let verdict: ReturnType<typeof verifyStatement>;
  try {
  ...
  const attestation: WorkspaceAttestation =
    typeof st.workspaceFile === "string" && typeof st.workspaceDigest === "string"
  ...
  if (pin === null) {
    ...
    switch (outcome.kind) {
      ...
      case "failed":
        ...
        reasons.push(
          `Bundle ${record.bundleId} verified, but pinning machine ${machineId}'s signing key failed (${outcome.detail}). The bundle was applied — a local file problem must not undo a verified pull — but until the pin store is writable again, every pull trusts the hub afresh for this machine.`
        );
        break;
      case "conflict":
...
export async function runFetchStage(
  ...
  return stageOk(
    { extractDir, manifest: bundleManifest, workspaceAttestation: sigOutcome.attestation },
    reasons
  );
}
