...
function sameFingerprint(a: string, b: string): boolean {
  const norm = (s: string): string => s.replace(/[^0-9a-fA-F]/g, "").toUpperCase();
...
export async function hubTrust(opts: HubTrustOptions): Promise<HubTrustOutcome> {
  ...
  if (opts.machineId !== undefined) {
    ...
    if (!target) {
      return refuse(
        "no-such-machine",
        `No machine ${opts.machineId} is registered on this hub.`,
        "Run `sesh-mover hub trust` with no arguments to list the machines this hub knows about, and confirm one of those.",
        warnings
      );
    }
    if (!target.signingPublicKey || !target.fingerprint) {
      return refuse(
        "no-key-published",
        `Machine ${opts.machineId} has published no signing key, so there is nothing to confirm.`,
        "That machine has not pushed since signing shipped, or its key file was unreadable when it last checked in. One push or pull from it publishes the key; then run this again.",
        warnings
      );
    }
    if (!opts.fingerprint || !sameFingerprint(opts.fingerprint, target.fingerprint)) {
      ...
      return refuse(
        ...
        opts.fingerprint
          ...
          : `Confirming a key requires the fingerprint from the other machine.`,
        `Run \`sesh-mover hub trust\` ON MACHINE ${opts.machineId} and read its fingerprint from there — over a channel that is not this hub. Then re-run with --fingerprint. If the two genuinely differ, do not confirm: either that machine re-minted its key, or something is publishing a key it does not hold.`,
        warnings
      );
    }
    ...
    if (outcome.kind === "failed") {
      return refuse(
        "pin-write-failed",
        `The fingerprint matched, but the pin could not be saved: ${outcome.detail}`,
        "Fix the permissions on ~/.sesh-mover and run this again. Until it is saved, this machine will keep treating that key as unconfirmed.",
        warnings
      );
    }
    if (outcome.kind === "conflict") {
      ...
      return refuse(
        "fingerprint-mismatch",
        `A different key is already pinned for ${opts.machineId}.`,
        "Confirm the fingerprint on that machine again before replacing the pin.",
        warnings
      );
    }
    return {
  ...
  if (conflicts.length > 0) {
    warnings.push(
      `${conflicts.length} machine(s) publish a key that differs from what is pinned here. Pulling a bundle from them will refuse until this is resolved. That is not proof of an attack — a machine that lost its key and re-minted looks exactly the same from here — which is why nothing resolves it automatically.`
    );
  }
...
function refuse(
  refusal: HubTrustRefusedResult["refusal"],
  error: string,
  suggestion: string,
  warnings: string[]
): HubTrustRefusedResult {
...
async function collectMachines(
  backend: HubBackend,
  hubId: string,
  pins: ReturnType<typeof readPins>,
  meId: string
): Promise<TrustedMachine[]> {
  const out: TrustedMachine[] = [];
