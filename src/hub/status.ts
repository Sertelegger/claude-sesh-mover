...
export async function hubStatus(opts: { cwd: string }): Promise<HubStatusResult> {
  ...
  if (lastLockSteal && lastLockSteal.kind !== "dead-holder") {
    warnings.push(
      `Another sesh-mover process took this project's lock at ${lastLockSteal.at} from a holder that was ${
        lastLockSteal.kind === "live-holder-past-ceiling" ? "still running" : "not identifiable"
      } (pid ${lastLockSteal.holderPid ?? "unknown"}), after it held the lock past the safety ceiling.` +
