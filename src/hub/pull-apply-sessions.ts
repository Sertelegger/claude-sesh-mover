import {
  appendFileSync, copyFileSync, existsSync, mkdirSync, readdirSync, statSync,
} from "node:fs";
 ...
 */
export async function runApplySessionsStage(
  input: ApplySessionsStageInput
   ...
   */
  const applyBundleSharedLayers = (): void => {
  ...
  if (record.type === "continuation" && !noAppend && bundleSession && existsSync(deltaPath)) {
    ...
    if (actualHash !== bundleSession.integrityHash) {
      reasons.push(
        `Continuation for thread ${threadId} failed its integrity check (bundle content doesn't match the manifest hash) — not spliced into the existing session; importing it as a separate session instead.`
      );
    } else {
        ...
        if (outcome.reason === "chain-mismatch") {
          ...
          if (mode === "adopt-hub" && divergence.adoptAvailable) {
            ...
            const adopt = await adoptHubBranch({
              ...
              adapters: getApplicableAdapters(
                bundleManifest.sourceClaudeVersion, claudeVersion
              ),
            });
            ...
            if (adopt.kind === "adopted") {
              ...
              reasons.push(
                `Adopted the hub branch for thread ${threadId} into session ${baseSessionId}; your local branch was preserved in full as session ${preservedSessionId}, which has no thread mapping and will therefore be published as its own thread on the next push.`
              );
            ...
            reasons.push(
              `adopt-hub is unavailable for thread ${threadId}: the continuation's anchor ${divergence.anchorUuid} is not present in the local session ${baseSessionId} (unrelated or compacted history) — importing the hub's branch as a separate session instead.`
            );
            divergence.resolution = "fragment";
          ...
          const preventive =
            outcome.reason === "recently-active"
              ...
              : "";
          reasons.push(
            `Continuation for thread ${threadId} could not be appended to the local session (${outcome.detail}) — imported as a separate session instead.${preventive}`
          );
        }
      }
    }
  ...
  if (!importResult.success) return { control: { kind: "fail", result: importResult }, reasons };
  st.lastAppliedIndex = i;
  st.lastBundleManifest = bundleManifest;
