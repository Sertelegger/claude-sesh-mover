...
export async function runResolveStage(
  ...
  // effect.
  let local: LocalProjectId | null = null;
  let linkTo: { projectId: string; note?: string } | null = null;
  ...
  if (linkTo) {
    ...
    if (linkTo.note) reasons.push(linkTo.note);
  }
  ...
  if (local === null) throw new Error("hub pull: project identity resolved to no link");
