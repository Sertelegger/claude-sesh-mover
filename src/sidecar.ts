import {
  closeSync, fchmodSync, fstatSync, lstatSync, openSync, readSync, unlinkSync, writeSync,
} from "node:fs";
 ...
 */
function isSymbolicLinkPath(dstPath: string): boolean {
  try {
...
function nameTakenError(dstPath: string): NodeJS.ErrnoException {
  const err: NodeJS.ErrnoException = new Error(
    `EEXIST: file already exists, open '${dstPath}'`
  );
  err.code = "EEXIST";
  err.syscall = "open";
  err.path = dstPath;
 ...
 */
export function copyToNewFile(srcPath: string, dstPath: string): void {
  ...
  try {
    if (isSymbolicLinkPath(dstPath)) throw nameTakenError(dstPath);
    ...
    try {
      ...
      for (;;) {
        ...
        if (read <= 0) break;
        let written = 0;
        while (written < read) written += writeSync(dst, buf, written, read - written);
      }
 ...
 */
export function copyToUniqueName(
  srcPath: string,
  nameFor: (attempt: number) => string,
  resolve: (name: string) => string,
  maxAttempts: number = MAX_SIDECAR_ATTEMPTS
): string | null {
