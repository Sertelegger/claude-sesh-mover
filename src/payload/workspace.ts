...
import {
  IGNORE_FILE_NAME, INCLUDE_FILE_NAME, PLUGIN_STATE_NAMES, PROJECT_DIR_NAME,
  ignoreFilePath, includeFilePath, isPluginStateName,
} from "../paths.js";
...
export class WorkspaceTargetNotEmptyError extends Error {
  ...
  constructor(targetPath: string) {
    ...
    super(
      `workspace target ${targetPath} exists and is not empty — pass force to unpack over it, overwriting files of the same name`
    );
...
// follow the other.
export function readIgnorePatterns(projectPath: string): string[] {
 ...
 */
export function readIncludePatterns(projectPath: string, diagnostics?: string[]): string[] {
  ...
  if (!st.isFile()) {
    if (st.isDirectory()) {
      diagnostics?.push(
        `${INCLUDE_FILE_NAME} is a directory, not a file — it was ignored entirely, so no re-includes are in effect.`
      );
    }
    return [];
  }
  if (st.size > MAX_INCLUDE_BYTES) {
    diagnostics?.push(
      `${INCLUDE_FILE_NAME} is ${st.size} bytes, over the ${MAX_INCLUDE_BYTES}-byte cap — it was ignored ENTIRELY, so none of its re-includes are in effect and every path it names was left out of this snapshot.`
    );
    return [];
  }
  ...
  if (all.length > MAX_INCLUDE_PATTERNS) {
    diagnostics?.push(
      `${INCLUDE_FILE_NAME} has ${all.length} patterns, over the ${MAX_INCLUDE_PATTERNS}-pattern cap — only the first ${MAX_INCLUDE_PATTERNS} are in effect and the rest were dropped.`
    );
  }
 ...
 */
function isNeverSegment(segment: string): boolean {
  return NEVER_INCLUDABLE.includes(segment.replace(/[.\s]+$/u, "").toLowerCase());
}

/**
 * A path or pattern split into segments, plus whether the raw text carried a
 * separator ANYWHERE — which is what makes a pattern *rooted*.
 *
 * `rooted` cannot be recovered from `segments.length`: `docs/` and `/docs`
 * both normalize to one segment, and both are rooted. Deriving it from the
 * length instead (the shipped bug) made `docs/` behave as the bare
 * match-at-any-depth form, so `dist/` also carried a `dist` directory nested
 * under any package — the permissive direction, on exactly the shape
 * `ignoredNotCarried` emits and `push.md` tells the model to paste verbatim.
 */
interface Segmented {
  segments: string[];
  rooted: boolean;
 ...
 * stricter.
 */
function toSegments(input: string, kind: "path" | "pattern"): Segmented | null {
  ...
  if (/^[a-zA-Z]:/.test(s)) return null; // drive-relative or drive-absolute
  ...
  if (s.startsWith("/") && kind === "path") return null;
...
export function isNeverIncludable(relPath: string): boolean {
 ...
 */
const patternCache = new WeakMap<readonly string[], Segmented[]>();

function parsePatterns(patterns: readonly string[]): Segmented[] {
  ...
  for (const raw of patterns) {
    ...
    if (p.segments.some(isNeverSegment)) continue;
 ...
 */
export function isReIncluded(relPath: string, patterns: string[]): boolean {
  const path = toSegments(relPath, "path");
  if (path === null || path.segments.some(isNeverSegment)) return false;
  const segments = path.segments;
  for (const { segments: pattern, rooted } of parsePatterns(patterns)) {
    if (!rooted && pattern.length === 1) {
      if (segments.some((s) => matchesSegment(pattern[0]!, s))) return true;
    } else if (
 ...
 */
export function mayContainReIncluded(dirRelPath: string, patterns: string[]): boolean {
  const dir = toSegments(dirRelPath, "path");
  if (dir === null || dir.segments.some(isNeverSegment)) return false;
  const segments = dir.segments;
  for (const { segments: pattern, rooted } of parsePatterns(patterns)) {
    if (!rooted && pattern.length === 1) return true; // bare: any depth below
    if (
      pattern.length > segments.length &&
...
export function isExcluded(name: string, patterns: string[]): boolean {
  for (const pattern of patterns) {
    if (pattern.includes("*") ? globMatch(pattern, name) : name === pattern) return true;
  }
  return false;
}
...
export function classifyDestination(
  targetDir: string,
  rel: string,
  expect: "file" | "dir" = "file"
): { ok: true } | { ok: false; reason: DestinationBlock } {
 ...
 */
export function forEachCarriedFile(
  root: string,
  rules: CarryRules,
  visit: (relPath: string, srcPath: string) => void,
  hooks?: {
  ...
  const admitPrefixes = new Set<string>();
  if (admit) {
  ...
  const walk = (dir: string, rel: string, insideExcluded: boolean): void => {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (isNeverSegment(entry.name)) {
        ...
        if (entry.isSymbolicLink()) hooks?.onSymlinkSkipped?.(childRel);
      ...
      const carried =
        !excluded || isReIncluded(childRel, includePatterns) || admit?.has(childRel) === true;
      if (!carried) {
      ...
      if (entry.isDirectory()) walk(srcPath, childRel, excluded);
      else if (entry.isFile() && carried) visit(childRel, srcPath);
    }
  };
 ...
 */
export function isCarriedPath(relPath: string, rules: CarryRules): boolean {
  const parsed = toSegments(relPath, "path");
  if (parsed === null || parsed.segments.some(isNeverSegment)) return false;
...
export function readCarryRules(projectPath: string, diagnostics?: string[]): CarryRules {
  return {
...
export async function unpackWorkspace(
  ...
  fileCount: number;
  symlinksSkipped: number;
  blocked: Array<{ path: string; reason: DestinationBlock }>;
  refused: string[];
}> {
