...
import {
  isNeverIncludable, snapshotWorkspace, type PayloadScope,
} from "./workspace.js";
 ...
 */
export function listTopLevelIgnored(projectPath: string): string[] {
  try {
...
export async function capturePayload(
  ...
  if (opts.wantWorkspace && scan.kind === "none" && projectExists) {
    ...
    if (ws.skipped) return { decision: "none", warnings };
    return {
      decision: "workspace",
      workspace: {
  ...
  if (opts.wantCarry && scan.kind === "remotes" && projectExists) {
    ...
    if (cap.reason !== "clean" && cap.reason !== "not-git") {
      ...
      warnings.push(
        `Uncommitted changes were not carried: ${cap.detail ?? cap.reason}. They will be picked up by the next ${noun} that has new session content.`
      );
    }
 ...
 */
function discoverIgnored(projectPath: string): string[] | undefined {
  if (existsSync(includeFilePath(projectPath))) return undefined;
...
function describeCarryCapture(
  ...
  if (meta.reIncludedCount > 0) {
    ...
    out.push(
      `${carried} ${meta.reIncludedCount} gitignored file(s) because .sesh-mover-include names them: ${shown}${more > 0 ? `, and ${more} more` : ""}. ${landed}.`
    );
  }
