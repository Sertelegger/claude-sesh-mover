 ...
 */
export function normalizeMemoryTarget(dest: string): string | null {
  let t = dest.trim();
  if (t === "") return null;
  ...
  if (t === "") return null;
  if (t.includes("://")) return null; // external link
  if (t.startsWith("#")) return null; // in-document anchor
  if (t.startsWith("/") || t.startsWith("\\")) return null; // absolute
 ...
 */
export function pointerTarget(line: string): string | null {
  ...
  if (depth !== 0) return null; // unterminated — not a link
 ...
 */
export function splitIndexLines(text: string): MemoryIndexLine[] {
  const out: MemoryIndexLine[] = [];
  let i = 0;
  while (i < text.length) {
 ...
 */
export function appendIndexLines(text: string, newLines: string[]): string {
  if (newLines.length === 0) return text;
  const eol = detectEol(text);
  const lines = splitIndexLines(text);
  if (lines.length === 0) return newLines.join(eol) + eol;
 ...
 */
export function unionMemoryIndex(local: string, incoming: string): MemoryIndexUnionResult {
  const localLines = splitIndexLines(local);
  const incomingLines = splitIndexLines(incoming);

  const localKeys = new Set<string>();
  const localProse = new Set<string>();
  for (const l of localLines) {
    if (l.key !== null) localKeys.add(l.key);
    else if (l.raw.trim() !== "") localProse.add(l.raw.trim());
  }
  ...
  for (const l of incomingLines) {
    if (l.key === null) {
      ...
      if (l.raw.trim() !== "" && !localProse.has(l.raw.trim())) droppedProse = true;
      continue;
    }
    if (seen.has(l.key)) continue; // two incoming lines for one key contribute one
    seen.add(l.key);
 ...
 */
function isUnsafeInlineCodePoint(cp: number): boolean {
  return (
    cp < 0x20 || // C0: \n, \r, \t, ESC …
    cp === 0x7f || // DEL
    (cp >= 0x80 && cp <= 0x9f) || // C1
 ...
 */
export function sanitizePointerText(text: string): string {
  let out = "";
...
function hasUnsafeInline(s: string): boolean {
  for (const ch of s) {
    if (isUnsafeInlineCodePoint(ch.codePointAt(0)!)) return true;
  }
  return false;
}
 ...
 */
export function formatMemoryPointer(
  title: string,
  target: string,
  description: string
): string | null {
  ...
  if (hasUnsafeInline(line)) return null;
  if (splitIndexLines(line).length !== 1) return null;
  if (pointerTarget(line) !== target) return null;
  return line;
}

/** Every memory file an index points at, deduped, in first-seen order. */
export function memoryIndexTargets(text: string): string[] {
  const out: string[] = [];
  const seen = new Set<string>();
  for (const l of splitIndexLines(text)) {
    if (l.key === null || seen.has(l.key)) continue;
    seen.add(l.key);
    out.push(l.key);
  }
  return out;
