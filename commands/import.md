...
10. Report the result.
    ...
    - If not resumable because of a version-mismatch or "session validation failed" error: offer to retry the step 9 invocation with `--no-register` appended. That imports the session content but skips the registry entry — the user won't get a `claude --resume` slot, but the JSONL is on disk. Note: after a `--no-register` import, a later normal import of the same bundle is skipped as a duplicate — re-run with `--allow-duplicates` to obtain a registered copy (this creates a second copy; smarter registered-state tracking is planned for 0.3.1).
