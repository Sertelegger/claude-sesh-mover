...
8. **Show the write set, then confirm.** Four of the six layers land under a session id this import mints, so they can collide with nothing. Two do not, and the dry run has already enumerated exactly what they will write, in `writeSet`. **Present that; do not derive it yourself and do not describe it in your own words.**

   - `writeSet.roots` — one per shared-namespace layer the bundle carries. `scope: "project"` is the target project's own directory (`memory/`); `scope: "machine"` is **shared by every project in this config dir** (`plans/`, which is why that layer is opt-in). `applied: false` means a flag declined the layer — say so, and give the count from `plansSkipped` / `memorySkipped`. These paths, and `memoryDir` after the run, are the ones to quote: never derive either yourself, because deriving the memory path means re-implementing the project-path encoding in markdown.
   - `writeSet.entries` — one per path. `kind: "create"` is a new file; `kind: "park"` keeps the user's file and saves the incoming copy beside it under this name; **`kind: "index-append"` edits a file the user already has** — that is `MEMORY.md`, the file that decides what a future session reads, so never bury it in a list.
   - `writeSet.total` — the authoritative count of paths.

   **Render `display`, never `path`.** Every entry carries both. `path` is the raw absolute destination and its last segment came out of the bundle, so it can carry a newline, an ANSI escape or an RTL override that restructures whatever you print it into. `display` is that same string already quoted and escaped — use it verbatim, in a code span.

   **Bound the list, and say what you withheld.** Show at most 20 entries. When `writeSet.total` is larger, say *"…and N more"* with N taken from `writeSet.total` minus what you showed — from the field, never from counting a list you have already cut down. Show every `index-append` entry regardless of the bound.
...
10. Report the result. Read what actually landed **before** offering anything to resume — `resumable` says the importer was allowed to register sessions, not that this run created any.
    ...
    - **Report the memory layer from the typed fields, not from the warnings.** A session's memories are only reachable through `memory/MEMORY.md`, so a transfer that lands them and leaves the index alone lands nothing usable (that is #49, measured: ten memory files on disk, referenced by nothing, reported as `success: true`).
      ...
      - `writeSet` — the same shape step 8 previewed, now describing what this run wrote. Present on every import, including when `total` is `0`, which means nothing landed outside the session's own directory. Read `entries[].kind` rather than inferring from the paths, and render `display` here too.
