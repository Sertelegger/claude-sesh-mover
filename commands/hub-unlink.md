...
**Exit codes:** keep branching on the parsed JSON, not on `$?`. `hub unlink` exits `0` both when it removed the link and when there was nothing to remove (`wasLinked: false` is an idempotent no-op, not a failure), and `3` when the project lock is held — the retryable class, which is the `lock-busy` branch in step 4: wait and re-run the same invocation, or offer `--force` there. `1` is a bad invocation or an unexpected failure. See "Exit Codes" in the skill doc for the full four-class table.
