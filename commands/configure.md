...
2. Present the current settings grouped by command:
   ...
   - **Hub defaults:** hub path (set via `/sesh-mover:hub-init`), workspace snapshot on push (on/off), git-carry on push (on/off), the local encryption *preference* (see `hub.encrypt` below — it is a preference, not the switch, so never present it as "encryption is on"; `/sesh-mover:hub-encrypt` reports the hub's actual setting), and the two automation switches — auto-push at session end, freshness notice at session start. Call the last two out by name when showing hub settings: they are on by default and take effect as soon as a project is linked to a hub project, so a user who has linked one is already being auto-pushed.
...
- `hub.encrypt` — true or false (default false). Encryption at rest is real as of 0.10.0, but **this key is not the switch and setting it does not turn encryption on**. The authoritative switch is an `encrypt` field in the hub's own `hub.json`, because a local-only flag has a silent failure — one machine that never set it keeps pushing plaintext into a hub the user believes is sealed, and nothing anywhere says so. Putting it on the hub means a machine that never set this key still refuses to push plaintext into a sealed hub. So this key is the local *preference*: `/sesh-mover:hub-encrypt --enable` writes the hub's switch and this key together, and setting this key on its own records an intention and changes no byte that leaves this machine. Say that plainly. Setting it while the hub is unsealed is not silently ignored either — every push then warns that the bundle went as plaintext and names `hub encrypt --enable` as the remedy, because a machine that encrypted unilaterally would push bundles the rest of the hub cannot read. Two facts a user asking about this key usually wants next: enabling encryption for the hub makes it private **going forward only** (everything already there stays readable by anyone with read access, forever — nothing rewrites another machine's bundles), and each machine's key lives at `~/.sesh-mover/identity.age`, whose loss makes every bundle encrypted to it permanently unreadable.
