...
2b. If `reachable: false` (a `success: true` result — `whereis` reports an unreachable hub, it does not refuse on one): **stop here and report that, before reading any other field.** `hubState` says which case it is — `"no-directory"` (an unmounted share, a synced folder that hasn't appeared on this machine yet, or a typo in `hub.path`), `"not-a-hub"` (a directory is there but carries no `hub.json`, so it was never `hub init`-ed or points at the wrong folder), or `"unresponsive"` (a filesystem call against the path timed out — the share is mounted but hung, so neither of the other two remedies applies and `hub.path` is almost certainly fine). Three things not to do with the rest of the result, because none of them were read from the hub:
