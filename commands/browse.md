---
name: browse
description: Browse exported sessions — view, import, or delete
---

You are running the sesh-mover browse command. Follow these steps:

1. List all exports:
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/dist/cli.js" browse --storage all --json
   ```

2. If the result has zero exports, respond with just: "No exported sessions found." Do NOT explain where you checked unless the user specifically asks.

3. If exports exist, present the results as a formatted table:
   ```
   #  Date        Name                  Summary                                        Platform  Source path                Sessions  Files       Storage
   1  2026-04-11  sesh-mover-design     Designing a session migration plugin           darwin    /Users/sam/dev/sesh-mover  1         —           user
   2  2026-04-09  fix-auth-middleware   Debugging JWT token expiry in auth middleware  wsl2      /mnt/e/GitHub/api-gateway  3         carry       project
   3  —           broken-bundle.tar.gz  (metadata unreadable: TAR_BAD_ARCHIVE: Unrecognized archive format)
   4  —           2026-04-08-api-notes  (metadata unreadable: manifest.json is not a sesh-mover bundle manifest)
   ```

   `exportedAt`, `sourcePlatform`, `sourceProjectPath`, and `sessionCount` are the **exporting** machine's values, read from each bundle's own `manifest.json` — for `.tar.gz`/`.tar.zst` archives as well as for directory exports. A bundle exported on WSL and browsed on a Mac lists as `wsl2` with its WSL path; never relabel it with the local platform.

   **The `Files` column** is `hasWorkspace` / `hasCarry`, and it is the one thing in this listing that is not about the conversation: a bundle can carry the project's own files beside the sessions — a whole-project `workspace` snapshot, or a git-diff `carry` of uncommitted work. Render `workspace`, `carry`, or `—` when both are `false`. Say it plainly when the user picks one: importing that bundle **can write files into the project directory**, though only if they also pass `--apply-workspace` / `--apply-carry` at step 5, which the import flow asks about separately. A bundle that carries neither can only ever add sessions.

   **Degraded entries.** An entry with `metadataAvailable: false` had unreadable metadata: every manifest-derived field is `null` (never a stand-in value), `sessions` is `[]`, and `metadataError` says why. That includes `hasWorkspace` and `hasCarry`, and there `null` is worth saying out loud rather than rendering as a dash: it means **nobody could check whether this bundle carries project files**, not "it carries none". Show the name and the error in place of the metadata columns, as in row 3 above — never blanks, zeros or a `—` in the `Files` column — and keep listing every other export normally. These entries sort last.

   A degraded entry is **either** an archive or a **directory** export whose `manifest.json` is unparseable, unsafe, or not a sesh-mover bundle manifest; the row shape is identical, so handle both the same way and don't infer the kind from `metadataAvailable`. Two things that look similar and are not: a directory with **no** `manifest.json` at all is not an export at all and never appears in the listing — only a bundle that *claims* to be one can degrade; and a directory in the project root whose `manifest.json` isn't ours (another tool's, a package's) is skipped silently rather than degraded, because there `manifest.json` is the only evidence the directory is an export in the first place. So a listing that omits something the user expected is not necessarily a bug — say what `browse` found, not what it might have skipped.

   **The zstd case.** A degraded entry whose **name ends in `.tar.zst`** is this case: `.tar.zst` bundles can only be read where the `zstd` binary is installed. Key off the name, not off the wording of `metadataError` — that field carries a free-text detail string that is not guaranteed to contain the word "zstd" (it usually will, and when it does it corroborates the name). Don't leave the user with an unexplained row — offer to install it:
   - macOS: `brew install zstd`
   - Debian/Ubuntu/WSL: `sudo apt-get install -y zstd`

   After the user accepts and the install succeeds, re-run step 1 so the affected archives show their real metadata.

   Reading `.tar.zst` metadata decompresses the whole bundle to a temp file first, so `browse` over many large zstd archives is IO-bound in total archive size — it can take a few seconds. That is expected, not a hang.

4. Use AskUserQuestion to let the user pick an export (each as a selectable option with name + summary), plus a "Cancel" option. Carry the `Files` answer into the option label — "+ workspace files", "+ carried git changes", or nothing when the bundle has neither — so the choice between two bundles is made knowing which of them can write into the project. A degraded entry has no summary to show (`sessions` is `[]`), so label it with its name plus a short "metadata unreadable" note instead, and say the file payload is unknown rather than absent — and keep it selectable: the bundle may still import fine.

5. Once an export is selected, use AskUserQuestion to ask what to do:
   - "Import to current project" — proceed with the import flow (dry-run, confirm, execute, report). Still offered for a degraded entry, but say up front that it will likely fail until the cause is fixed (install `zstd`, replace a corrupt archive, repair a directory export's `manifest.json`), since import has to read the same bundle `browse` just couldn't.
   - "View details" — show what the browse entry actually carries, and nothing beyond it: `name`, `path`, `exportedAt`, `sourcePlatform`, `sourceProjectPath`, `sessionCount`, `hasWorkspace`, `hasCarry`, `storage`, plus one row per entry in `sessions` (`sessionId`, `slug`, `summary`, `createdAt`, `lastActiveAt`, `messageCount`, `gitBranch`, `entrypoint`, `integrityHash`, and `type` when present — `"continuation"` marks an incremental bundle's delta session). The result is **not** the full manifest: it carries no included-layers list and no source Claude version, so do not promise either. For a **directory** export with `metadataAvailable: true` you may read `<path>/manifest.json` directly if the user asks for those — it holds `includedLayers` and `sourceClaudeVersion`; for a `.tar.gz`/`.tar.zst` there is no manifest on disk to read. For an entry with `metadataAvailable: false` there is nothing to show at all (`sessions` is `[]`): report the `metadataError` instead, plus the zstd install offer above when it applies — and do **not** fall back to reading a degraded directory's `manifest.json` yourself, since that read is exactly what failed. Showing the user the raw broken file is fine if they ask for it, but present it as the damaged file it is, never as metadata.
   - "Delete" — there is **no** sesh-mover delete command; deleting is a plain filesystem removal you perform yourself, so be exact about it. Confirm with AskUserQuestion ("Yes, delete" / "Cancel"), showing the entry's `path` verbatim in the question so the user approves the literal path. Then remove that one artifact and nothing above it: `rm -rf "<path>"` for a directory export, `rm "<path>"` for a `.tar.gz`/`.tar.zst`. Never widen the target — not the containing `.sesh-mover` directory, not a glob, not a parent — and if `path` is missing or does not end in the export's own `name`, stop and ask the user rather than guessing.

6. After completing an action, if the user chose "View details", offer the import/delete options again for the same export.

**Exit codes:** keep branching on the parsed JSON, not on `$?`. `browse` exits `0` whenever it produced a listing — including one whose entries are `metadataAvailable: false`, since an unreadable bundle is data about that bundle, not a failed command — and `1` only when the command itself failed, in which case stdout carries an `ErrorResult` (or, for an argument Commander rejected, nothing at all and a message on stderr). See "Exit Codes" in the skill doc for the full four-class table.

**Invocation:** `${CLAUDE_PLUGIN_ROOT}` is set by Claude Code inside plugin command execution — use it as-is in the bash invocation above; do not search the plugin cache. The flag set documented in this file (in both the main invocation and any conditional branches) is authoritative — do not run the CLI with `--help` or with no arguments to discover its surface.
