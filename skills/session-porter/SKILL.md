...
**Continuation append (pull):** when a pulled bundle continues a session this machine already has, `pull` splices the new entries onto the end of that local session instead of creating a second, truncated "continuation" session — so the thread stays one resumable transcript. Successful splices appear in `appended: [{ threadId, baseSessionId, entriesAppended }]`; those sessions are **not** in `importedSessions` (nothing new was created). The splice is refused, with a warning and a normal fragment import as the fallback, when the local session's entry chain doesn't line up with the continuation, or when the local file was written in the last 5 minutes (a live Claude Code session may still be appending to it). `--force-append` overrides only the second case; `--no-append` (or `hub.pullAppend: false`) disables the whole path. Content always arrives either way — the only difference is one session or two.
